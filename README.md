...
| 🔗 **Group Analyzer** (BETA) | Where an Entra group is *actually* used. Paste a group name, a UPN or an object ID and get every place it is referenced across **23 services**: **Entra ID** (group nesting, administrative units, directory roles incl. PIM-eligible ones, enterprise application assignments, group-based licensing, the authentication methods policy, Conditional Access, access packages, admin-consent reviewers, and — for a user — what they have actually registered), **Intune** (enrolment device-limit and platform restrictions, compliance policies, configuration profiles across device / settings catalog / ADMX, PowerShell and shell scripts and remediations, app protection, app configuration, app assignments, Autopilot profiles, Windows update profiles), **Microsoft 365** (M365 group / provisioned team, and the Purview **container sensitivity label** on the group — the only part of Purview with a Graph API; DLP, retention, label publishing, insider risk and communication compliance are Security & Compliance PowerShell only and are called out as such) and **Azure RBAC** across every subscription and management group you can read, down to resource scope. Hits inherited from a **parent group** are labelled as such — anything targeting the parent reaches these members too. **Sweep every group** runs the same checks tenant-wide and produces a group × service count table plus the list of groups **nothing references**. Each area (Entra / Intune / M365 / Azure) is opt-in and asks for its own permissions at the moment you tick it; anything that failed or was never granted is listed by name under **Not read**, with the permission *and* the directory/Intune role it needs. A sweep defaults to **only the groups Conditional Access actually assigns** — taken from the include and exclude groups of every policy already in memory, so no directory enumeration and no dependence on tenant size; an id a policy names but the directory can no longer resolve is kept and flagged as a **dangling reference**, because that assignment targets nobody. Other scopes are first 100/250/500 or every group, and any of them can be narrowed by name (**starts with / ends with / contains**) — a prefix like `CAB-SEC-` cuts a 20 000-group tenant to the set worth looking at, and “groups in scope” then counts matches rather than raw groups; it runs server-side where Graph supports it and falls back to filtering locally where it does not. Clicking a group opens a **popup** built from the sweep data — no second scan — with a **Deep analyze** action for the parent-group expansion a sweep does not do, and **← Back to the sweep** to return to the full list without re-running it. Exports **Markdown**, a standalone **HTML** report and **CSV**. | no |
...
- **Workspaces (2.0).** A branded top bar, compact navigation rail, searchable tool library and persistent tool tabs keep the existing tools close at hand. Policy details open in a wide, centered dialog. Tool headings, actions and panels share a consistent layout.
- **Light and dark.** Follows the device by default; Theme in the signed-in account menu cycles Auto → Light → Dark (the sign-in screen retains its theme button). Exports are always captured in light theme.
...
js/groupuse.js        Group Analyzer: the 22-service source registry + exports
js/baselineData.js    the CloudFellows baseline catalog (R26.6 / v3.x)
...
The day-to-day release flow:

```bash
# work lands on the beta branch (never directly on main)
git checkout beta

# deploy to the beta site and test at https://nurejev.github.io/enca-beta
git push beta beta:main
#   → the page wears the red "BETA — not production" ribbon

# happy? promote to production
git checkout main
git merge beta
git push                       # GitHub Pages deploys enca.limon-it.nl
...
- **The baseline catalogs are content.** `js/baselineData.js` and `js/groupTemplates.js` hold the CloudFellows CA numbering and group naming convention (`CAB-SEC-U-*`, `CAD-SEC-U-DG-*`). If your convention differs, fork those files — the tools read the convention from them, not from the code.
...
| `DeviceManagementServiceConfig.Read.All` | Group Analyzer — Intune enrolment restrictions and Autopilot deployment profiles |
| `DeviceManagementScripts.Read.All` | Group Analyzer — Intune PowerShell scripts, macOS shell scripts and remediations. A **separate** scope: `DeviceManagementConfiguration.Read.All` does not cover them, which is why scripts return 403 without it |
...
**Azure is not a Graph permission.** Group Analyzer's Azure area reads Azure RBAC through Azure Resource Manager, a different resource with its own token: `https://management.azure.com/user_impersonation`. It is consented in the browser the moment you tick the Azure area, never bundled into the Graph consent request, and there is nothing to add to the app registration for it — the signed-in user simply needs a role that can read role assignments (e.g. **Reader**) on the subscriptions or management groups they want to see.
...
Several community projects shaped ENCA, and all of them deserve the credit:
...
- **[Microsoft Cloud Group Analyzer](https://github.com/jasperbaes/Microsoft-Cloud-Group-Analyzer)** — by **Jasper Baes**. The question behind the **Group Analyzer** tool: where is an Entra group actually used, across Entra, Intune, Microsoft 365 and Azure? His Node script answers it from a workstation with an app registration and a client secret; ENCA answers it in the browser with delegated permissions and adds group-based licensing, administrative units, PIM-eligible role assignments, admin-consent reviewers, Windows update profiles and management-group RBAC on top.
- **[Conditional Access Baseline](https://github.com/j0eyv/ConditionalAccessBaseline)** — by **Joey Verlinden**. A deliberately minimised community baseline built on the Microsoft Conditional Access framework, bundled here as a second catalog so a tenant can be measured against it exactly like the CloudFellows one. The policy set, personas and naming convention are his; ENCA only reads and compares.
- **[M365 Security Toolkit](https://github.com/cdell2222/m365-security-toolkit)** — by **Charlie Delmotte**. The checks behind the **Cross-tenant** tab of 🛡 Checks: default and partner cross-tenant access, trusted external MFA and devices, cross-tenant sync, automatic redemption and dead partner entries (MIT).
