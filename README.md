...
| 🔍 **Gap analyse** | Users × policies impact matrix: which policies apply to whom, who bypasses one through an exclusion (and why), whether that bypass is covered elsewhere, and who gets no MFA at all. Filter by group or user type (the matrix columns then narrow to policies in scope of that group), then export the filtered set as a standalone HTML report. Includes a **What-If simulator** — like the Conditional Access What-If in the Entra portal: enter a user and optional sign-in conditions (app, platform, client app, sign-in/user risk) and see which policies apply, which don't and why, and the combined grant / block / session outcome. | no |
