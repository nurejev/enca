...
| 🔗 **Group Analyzer** (BETA) | Where an Entra group is *actually* used. Paste a group name, a UPN or an object ID and get every place it is referenced across **22 services**: **Entra ID** (group nesting, administrative units, directory roles incl. PIM-eligible ones, enterprise application assignments, group-based licensing, the authentication methods policy, Conditional Access, access packages, admin-consent reviewers, and — for a user — what they have actually registered), **Intune** (enrolment device-limit and platform restrictions, compliance policies, configuration profiles across device / settings catalog / ADMX, PowerShell and shell scripts and remediations, app protection, app configuration, app assignments, Autopilot profiles, Windows update profiles), **Microsoft 365** (M365 group / provisioned team) and **Azure RBAC** across every subscription and management group you can read, down to resource scope. Hits inherited from a **parent group** are labelled as such — anything targeting the parent reaches these members too. **Sweep every group** runs the same checks tenant-wide and produces a group × service count table plus the list of groups **nothing references**. Each area (Entra / Intune / M365 / Azure) is opt-in and asks for its own permissions at the moment you tick it; anything that failed or was never granted is listed by name under **Not read**, with the permission *and* the directory/Intune role it needs. In a sweep, clicking a group opens a **popup** built from the sweep data — no second scan — with a **Deep analyze** action for the parent-group expansion a sweep does not do. Exports **Markdown**, a standalone **HTML** report and **CSV**. | no |
