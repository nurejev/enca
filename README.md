...
| 👥 **Assign groups** | Replace, add to, or reset the include/exclude persona groups — scoped to the selected policies or to **every policy in the tenant** (a blanket break-glass or service-account exclusion). Any group can be targeted, not just the baseline personas: search by name prefix or paste an object ID. Tenant-wide runs of the *replacing* actions need a typed `ALL` confirmation; *ADD to EXCLUDE* is additive and does not. Missing groups are created via Graph: **assigned groups are always role-assignable** (`isAssignableToRole: true` — immutable, so it is set at creation), while **dynamic groups are created dynamic with their membership rule intact** and are not role-assignable, since Entra forbids the combination and the rule is the point of those groups. | **yes** |
