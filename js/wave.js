  ...
  function render(res, opts = {}) {
    ...
    if (res.retired && log) {
      ...
      retHtml = `<div class="list-card wo-card">
        ...
        <p class="mini" style="margin:0 0 8px">${rt.policies.map((P) => `<span class="pol-link" data-polid="${esc(P.id)}">${esc(policyLabel(P))}</span> <span class="muted">— ${P.hasDev ? "compliant device or approved app" : "approved app only"}${P.hasApp ? " or app protection policy" : ""} · reaches ${P.reach}</span>`).join("<br>")}</p>
