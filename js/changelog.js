...
const CHANGELOG = [
  {
    ...
    items: [
      { kind: "fixed", tool: "CA groups", text: "A real tenant answered <b>every</b> disableNesting write with <code>400 Request_BadRequest \u2014 \u201cUnexpected request made to property 'disableNesting' of resource 'Group'\u201d</code>, on the create body as well as the PATCH. That wording is the directory saying it does not know the property NAME \u2014 not that it dislikes the value, and not \u201csettable only at creation\u201d, which is what the field reports had led us to assume. Checking Learn again settles it: the v1.0 <i>Update group</i> page names the property once, in a permissions note; the beta page does not list it as updatable; and <b>neither version's group resource carries it</b> in the property table or the JSON representation. The permission is documented, the property is on no published schema. It is not generally available." },
      { kind: "improved", tool: "CA groups", text: "So it is <b>off by default</b> on every create path, and offered as a tick in \u2460 Create missing groups and \u2461 Build a group manually instead. Since build 284 it was applied automatically everywhere \u2014 right in principle, and in a tenant without the property it meant a red failure line under every single group created, for a setting the panel had promised in its footnote. One constant, <code>CaGroups.NESTING_GA</code>, flips the default back the day Microsoft ships it, without touching a caller. \u2466 Migrate's nesting box follows the same default, and the nesting permission is requested only when the box is ticked." },
      { kind: "fixed", tool: "CA groups", text: "The request also goes to <b>v1.0</b> now, for this property only. ENCA talks to <code>/beta</code> everywhere, and the only Learn page that names disableNesting is the v1.0 one \u2014 so a tenant that HAS the property should not be refused merely because we asked a version whose documentation does not carry it. Every other call is unchanged." },
      { kind: "fixed", tool: "CA groups", text: "<b>\u2467 Disable nesting no longer walks you into a destructive dead end.</b> When the property was unknown it reported the failure and then offered its fallback: rename the group aside, recreate it, move the members, repoint every policy \u2014 setting disableNesting in the create body, which is the request that had just been refused. A rebuild to arrive at the identical error. That refusal is now told apart from an ordinary one: the action stops being offered for the rest of the session, the report explains why nothing here can set it, and it points at a restricted management administrative unit, which limits who can change the members at all and is available today. An ordinary refusal \u2014 a group that already holds nested groups \u2014 still offers the recreate, because there it works." },
      { kind: "fixed", tool: "Import", text: "The \u267b\ufe0f preflight asked for <code>disableNesting</code> inside the batch that looks up the file's groups by name. On a tenant without the property that whole sub-request returns 400, leaving the lookup empty \u2014 which does not read as \u201ccould not check\u201d, it reads as \u201cnone of these groups exist in this tenant\u201d. A wrong answer rather than a failure, and the panel would have reported nothing being reused when everything was. Nesting is read on its own pass now, and anything unreadable stays <i>not reported</i>." },
