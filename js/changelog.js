...
const CHANGELOG = [
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "All tools", text: "<b>Light and dark are separate artwork</b>, not one file with the colours filtered. The dark variant is swapped in by the stylesheet rather than by script, so it is already correct on the sign-in screen before the app has finished booting — the moment where a wrong-theme logo is most visible, because there is nothing else on the page yet. Both paint their own background, so the mark carries on any header colour, and the stylesheet rounds them like an app icon rather than leaving a square patch." },
      { kind: "improved", tool: "All tools", text: "<b>The framing is the one thing that changed about the artwork.</b> As supplied it is a 1024px icon tile that spends its outer third on the dashed orbit ring — which reads beautifully at that size and is illegible at the <b>34 pixels</b> the header actually uses, where the shield lands on about twelve of them. The viewBox is cropped to the inner disc: every element is still there, nothing is redrawn, and the original framing is recorded in a comment at the top of each file so it can be restored in one edit." },
      { kind: "fixed", tool: "All tools", text: "The two files shared every internal id — <code>glow</code>, <code>softglow</code>, <code>bgg</code>, <code>goldg</code>. SVG ids are document-global, so with both marks on one page the second one's gradients would have silently lost to the first and the dark mark would have rendered in the light palette. Each file's ids are now suffixed. The old Limon-IT mark and favicon are removed rather than left in <code>assets/</code> for somebody to wire up again by mistake, and the licence note names both marks as excluded from the MIT terms." },
    ],
  },
  {
    ...
    items: [
      { kind: "new", tool: "Gap analyse", text: "A <b>coverage flow</b> above the results, and usually the first thing worth reading. Five stages, each a subset of the one above: everyone <b>in scope</b> → <b>targeted</b> by at least one active policy → covered by one that is actually <b>enforced</b> → required to do <b>MFA</b> → holding the <b>licence</b> their own policies oblige. The four summary cards each counted a different thing against a different denominator and none of them said how many people were in the tenant to begin with — so the one number everybody actually wants, <i>how much of the tenant does this cover</i>, could not be read off the screen at all." },
      { kind: "new", tool: "Gap analyse", text: "<b>The drops are the finding, not the totals.</b> Every bar is drawn against the same denominator, so the shape of the funnel is readable without doing arithmetic on five percentages — and a tenant with 900 targeted users and 40 covered by an enforced policy has a <i>report-only backlog</i>, not a coverage problem, which is a distinction no single number can carry. Users reached by <b>no active policy at all</b> and users targeted <b>only</b> by report-only policies are called out under the funnel: for the first group Conditional Access is not weak, it is absent." },
      { kind: "new", tool: "Gap analyse", text: "<b>Every drop clicks through to exactly those users</b> — and clicking again clears it, because a filter you cannot switch off is a trap on a list this long. The drop sets are deliberately <b>not</b> the summary cards' sets: “No MFA from CA” counts everyone without MFA including the users no policy reaches, while the funnel's MFA drop counts only those a policy <i>does</i> reach and still does not ask. Reusing the card filters would have put a number on the funnel that opened a different number of rows, so the predicate that counts a drop is the same one that filters the list. A stage with no drop offers no drop link — only the first row stays clickable, and it clears the filter." },
      { kind: "new", tool: "Gap analyse", text: "<b>The licence stage reads its own data.</b> The assigned licences ride along on the <code>/users</code> read the tool was doing anyway and the subscribed SKUs are one more call, both covered by permissions it already holds — so there is no extra consent and no dependency on 🎫 Licence gap having run first. The verdict comes from <code>LicGap.licenceOf</code>, now extracted so both tools share <b>one definition of licensed</b>; a second implementation is how two screens end up disagreeing about the same user, which has happened here before. P2 is required where a risk-based policy targets someone, P1 otherwise, and report-only counts — the obligation follows targeting, not enforcement." },
      { kind: "fixed", tool: "Gap analyse", text: "<b>The first draft of the licence read would have been wrong on every real tenant, and confidently so.</b> It resolved the users through <code>/directoryObjects/getByIds</code>, which returns only the common user property set and does not honour <code>$select</code> — so <code>assignedLicenses</code> came back absent for everybody, every user classified as unlicensed, and the stage reported that as a <i>read</i> rather than as a failure. Demo mode hid it completely, because the fixtures were hand-shaped with the fields present. The two fields now ride along on the <code>/users</code> read the tool was already doing, which is what makes the “no extra call” claim true; a user record carrying neither field is treated as NOT READ for that user, never as unlicensed." },
      { kind: "improved", tool: "Gap analyse", text: "<b>Not read is not zero.</b> A refused or broken licence read leaves that one stage saying <i>not read</i> with every other stage unaffected, because “we did not look” and “nobody is licensed” are opposite findings. Targeted users with no licence record at all — guests, or a capped read — are left out of the stage and counted separately rather than rounded up to licensed. The funnel itself is a pure function of the report, testable with <code>node</code> alone; building it caught a real bug in its own first draft, where the licence stage was measured against the MFA row instead of the targeted one and reported a drop of 1 while the list it opened held 2. 🔍 Gap analyse is v1.8." },
      { kind: "improved", tool: "Gap analyse", text: "Two smaller things the funnel dragged into the light. The <b>exported report</b> named its own filter from a three-entry lookup, so exporting under a coverage filter produced a deliverable whose scope line read “filtered:” and then nothing — every filter is nameable now, and the funnel in the export describes the whole <b>run</b> rather than the filtered subset, which would have made the drop set its own denominator and read 100% at every stage. And the per-user licence entitlement is deliberately <b>stripped</b> from the data embedded in that file: the report renders nothing from it, and this is the file that gets shared." },
      { kind: "improved", tool: "Gap analyse", text: "The group, user-type and search controls narrow the list <i>below</i> the funnel without narrowing the funnel, so a row reading −7 could open a list of 2. Clicking a drop now clears those three, because the funnel's promise is that −7 shows those seven — and while any of them is active the funnel says outright that the list is filtered further. The alternative, quietly recomputing the funnel against whatever is on screen, would have made the headline number move every time somebody typed in the search box." },
      { kind: "new", tool: "All tools", text: "<b>Every row in 🚚 Waiting for production now carries a test checklist.</b> <i>Why</i> already said what the risk was and what would have to be true for an item to graduate; it did not say how to find out, and an item whose graduation condition nobody knows how to check either ships untested or never ships. The steps name the tenant state each one needs and the outcome you should see, so a step can fail rather than be nodded through — and where a check needs a tenant nobody has to hand, the step says so, because knowing which check was skipped is worth more than a list pretending all of them were run. The three items already queued have been backfilled; an item without one is now marked as unfinished in the table itself." },
    ],
  },
  {
    build: 25158, date: "2026-08-19", title: "Eight of the nine reached production 287 — and the ninth says why it did not",
    items: [
      { kind: "improved", tool: "All tools", text: "Queue items <b>61</b> (🎫 Licence gap), <b>62</b> (🚦 Sign-in failures gathers the Conditional Access interrupts), <b>63</b> (📥 Import finishes the job on the groups it reuses — R04), <b>64</b> (🗄 Backup and 📄 Create documentation produce the baseline catalog on a baseline tenant), <b>65</b> (🧬 the CloudFellows rename — R35), <b>66</b> (🌐 named-location findings — R37), <b>68</b> (🔢 permanent T-numbers — R33) and <b>69</b> (🧬 both baselines as one table) are live in <b>production build 287</b>. Removed from the queue; the numbers are retired rather than reused. <code>productionBuild</code> moves to v1.0.287." },
      { kind: "improved", tool: "Roadmap", text: "The four cards with roadmap references now read <b>live · beta NNNNN · production 287</b> on this channel and <b>live · build 287</b> on main — R04, R35, R37 and R33. Both channels carry their own copy of the roadmap, and step 3 of the promotion routine is the one that gets missed because the port is finished and working by then; eight cards had drifted that way on 2026-08-17." },
      { kind: "improved", tool: "Restricted AUs", text: "<b>Item 67 (R28 — custom groups in the persona vaults) was held back</b>, and its card and its queue row both say so. It is the only change in the batch that alters where a <b>write</b> puts a group object, and a persona vault is an authorisation boundary: file a group in the wrong one and that persona's scoped administrator can edit another persona's exclusions. An empty mapping leaves every existing routing decision byte-for-byte as it was, so the risk is entirely in what an operator states — which is exactly why it wants a real tenant behind it first. The code is unchanged and still running here." },
      { kind: "improved", tool: "All tools", text: "The promotion queue's header gains a <b>third</b> reading of a card that carries no production clause. Two were documented — the tool is genuinely beta-only, or somebody skipped step 3 — and R28 is neither: it is finished, running, and deliberately left out of a release its neighbours went in. That case now has its own wording, <b>“beta NNNNN · held from production”</b>, because a held item and a forgotten one look identical on the card, and “no production clause” must never be something a reader has to interpret." },
      { kind: "improved", tool: "All tools", text: "Production 287 carries <b>two gaps in the T-numbering</b>, T21 and T29, and they are correct: they belong to 📐 CIS Benchmark and 📖 Baseline guide, which are still only here. The numbers are theirs from the day they were assigned, whether or not production carries the tool — when those two graduate they arrive as T21 and T29, not as the next free numbers. Both remain queued, and neither can move without its tool." },
    ],
  },
  {
