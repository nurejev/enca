...
const CHANGELOG = [
  {
    ...
    items: [
      ...
      { kind: "new", tool: "PIM baseline", text: "🗺 Regions (multi-region profile): regions are never in the catalog — only a template is. Load the customer's regions.csv (code, name, attribute, value, devicePrefix, autopilotTag, itLead, approvers, timezone; JSON accepted; the example and a template are one click; the rows stay in this browser per tenant) and every row (REG = its code) becomes AU-REG-Users and -Devices (dynamic on the row's attribute, one object type each), AU-REG-Groups (assigned), PIM-SG-REG-Helpdesk and -Ops (role-assignable, GroupTier2) and the plain -Approvers group, eight eligibilities scoped to the region's units — Helpdesk, Password, Authentication and License Administrator for the first line on users; User Administrator on users, Groups and Teams Administrator on groups, Cloud Device Administrator on devices for the second line, only roles Entra can scope to a unit — plus the Intune scope groups INT-SG-USR/DEV-REG-All, the tag INT-TAG-REG and the assignments INT-RBAC-HelpDesk/Ops-REG. Each object is compared with the tenant: unit present with the expected rule, group present and role-assignable, each scoped eligibility present (one held at tenant scope is called out as wider than the region); Intune is not read yet (T53). The read now also takes the administrative units (AdministrativeUnit.Read.All, on demand) and the PIM-SG / INT-SG groups that are not role-assignable." },
      { kind: "new", tool: "PIM baseline", text: "⬇ Regions file writes the ticked regions with the template and the custom Intune role for tools/pim/New-PimRegions.ps1 (new): per row the three units, the persona and approver groups (approvers from the row), the PIM-for-Groups settings on Member (GroupTier2, rule by rule through Graph), the scoped eligibilities for a year, the Intune scope groups, scope tag (auto-assigned from the device group), the custom role INT-ROLE-Regional-Ops (actions the tenant does not know are left out and named) and the two role assignments; at the centre AU-RM-Admins as a restricted management unit, with -RmauMembers filling it. WhatIf by default, -Apply to create, -Only for one region, -FixRules to rewrite a differing rule, -SkipIntune; nothing is ever deleted. tools/pim/regions.csv carries the two demo regions of cloudfellows.dev, EU-NL and EU-DE; pim-baseline.json now carries every profile's groups so each has a real reference there; pim-baseline.small.json and .multi.json per profile." },
      { kind: "improved", tool: "PIM baseline", text: "New-PimBaseline.ps1 -SkipOrchestrator stops after the groups, the protected list and the resolved file — for the baseline tenant, where the role settings are set in the PIM portal and EasyPIM is not used." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { kind: "improved", tool: "Help", text: "Production is ENCA 2.0 — v2.0.0, build 322 on enca.limon-it.nl — carrying everything queued for production up to beta 25495 (queue 230–278); the queue now holds only what stays here. This channel moves to the 2.0 series: it reads v2.0.1-beta.N, its builds continue at 32301, and the 25xxx releases below are in the 1.x Archive. Below, the 2.0 release notes as production shows them." },
      { kind: "new", tool: "Workspaces", text: "One layout for every tool. A left rail with the tools in work order and a searchable library, a branded header with the logo on its medallion, list tools drawn as a list beside an inspector that keeps the open row highlighted, panes that scroll on their own and hand over to the page at their end, a ▾ at the end of every tool title that folds the tool head away, flat icons, and a contrasting active tool and tab. Policy building blocks and Checks have their own rail shortcuts." },
      { kind: "new", tool: "Home", text: "🏠 An overview on the home page: when the policies were read, the policy context (report-only, recently modified, baseline match, unique exclusion references), Worth a look first — findings with their evidence state, an evidence panel and a policy filter that carries into 🗂 Policies — your own check runs, a configuration map, what changed since the previous refresh, and upcoming Microsoft changes. It reads nothing beyond what sign-in already read, says Unknown rather than zero for anything unread, and never scores provisionally." },
      { kind: "new", tool: "Policies", text: "✎ Edit this policy on its card — the first button in the card's action row. The card's sections become forms, the name becomes a field, a bar counts what changes with a ± Diff and a preflight, and only the changed sections are saved and read back. Moving a policy from an authentication strength to Require MFA clears the strength, and a strength together with Require MFA is refused before the write, as Entra does." },
      { kind: "new", tool: "Policy building blocks", text: "🏗 Policy builder (T41), the first tab of Policy building blocks: build a new policy guided — persona, resources, conditions and controls with hints from your own tenant, a What-If preflight, a change-plan export — born in report-only. The 🗂 picker offers Clone and Edit, and panels you open stay open." },
      { kind: "new", tool: "Sign-in", text: "🪪 Register ENCA in your own tenant from the browser: a four-step wizard creates the single-tenant registration, its service principal and optionally admin consent, each step read back, and makes it the default sign-in. It lives in the ⚙ connection panel on the sign-in card and in the account menu, and only offers itself where the registration in use can actually carry it." },
      { kind: "new", tool: "Policies", text: "👥 Assign can target guests and external users as a third kind of target, and a directory-role edit no longer drops a policy's guest clause. A persona filter bar narrows the list, and Clear now clears the view, not only the selection." },
      { kind: "new", tool: "Policies", text: "🧹 Housekeeping gets ⇄ Switch over for a version change an import left half-done, a Reviewed tick with Mark all reviewed and Select all, and version names written with a hyphen. Deleting policies runs like an import: a row per policy, ■ Stop, and a report." },
      { kind: "new", tool: "Conditional Access groups", text: "🔀 Merge groups that share a display name: members first, then every Conditional Access policy repointed and read back, and only then the duplicate renamed aside or deleted — behind a review, a typed confirmation and a JSON backup." },
      { kind: "new", tool: "Baseline", text: "⬇ Export baseline from the baseline page, per persona and with its dependencies — the same export as 🗂 Policies' backup. 🧱 Update the catalog works from the baseline tenant itself, with Select all, and the Newer-than-baseline chip and the catalog panel count the same policies. Outside the baseline tenant the CA number and version decide; a tenant's own assignments and exclusions are never a difference." },
      { kind: "improved", tool: "Baseline", text: "The CloudFellows catalog is revised from CloudFellows.dev (22 September 2026): B2B direct connect is out of the session policies, and the guest-exclusion types and versions follow upstream." },
      { kind: "improved", tool: "Checks", text: "Guests and external users are judged against what they can actually satisfy. Every finding says whether the control 🚫 blocks them or 🕳 misses them, the remedy is split into what to exclude and what to create, a strength no stronger than Require MFA is a swap in place, and a stricter one gets a ready-made companion policy. Guests are followed through included groups, a Temporary Access Pass is not counted as a guest method, and an unconditional block counts as MFA coverage." },
      { kind: "improved", tool: "Checks", text: "📘 Microsoft Learn checks read cross-tenant trust from the default and every partner, know Windows Cloud Login as a token-protection target, keep the token-protection device filter to Entra-joined devices, treat the retired approved-client-app control as read-only, and name external user types no MFA policy reaches. A shared-device matrix holds Teams Rooms and Surface Hub against the controls policies demand, with a finding and a fix under every blocked cell. The tab waits for ▶ Run checks and keeps its result across a tab switch." },
      { kind: "improved", tool: "Checks", text: "🧀 Bypass & Swiss cheese: MFA and legacy authentication are judged by their real scope, OR grants get verdicts by what the alternative is, and new checks cover security info registration, device code flow, duplicate group names and allow-list blocks — with recommendations written as where, what to change and why, and a sign-in frequency column." },
      { kind: "improved", tool: "Exclusion analyzer", text: "🚪 Configured versus effective exclusions, the whole external-user clause, and provenance paged through nested groups. The analyzer opens list-first with an odd-one-out grid: cell evidence on click, a keyboard-navigable grid, capped headers, filters as chips, a pair card when a row and a column are pinned, and the member button on its own line. It shares one coverage comparison with 🔍 Gap analyse, so the two cannot disagree, and policy IDs travel through the results." },
      { kind: "improved", tool: "Gap analyse", text: "🔍 Cohorts of users whose state is identical across every policy, and 🎫 Licences as three separate measures — demand, purchased capacity and assigned entitlement. Results are bound to the run that made them: a refresh invalidates them, an evidence strip says what they rest on, and a result is published whole or not at all." },
      { kind: "fixed", tool: "Who is … to CA", text: "🕵 Policies scoped to specific external-user types now resolve for a user, and a scope ENCA cannot decide shows as its own fourth state instead of being guessed — the same answer in ⚖ Compare users, ⚡ CA validator and 🔍 Gap analyse." },
      { kind: "fixed", tool: "Sign-in log", text: "🚦 The tab keeps a ceiling on what it holds for one window, so Defender hunting with non-interactive sign-ins no longer crashes the page on a large tenant; the tools that share the window say so in the same words." },
      { kind: "fixed", tool: "Policies", text: "Editing a policy with an insider-risk condition is no longer stopped: Adaptive Protection is recognised from Insider Risk Management and the Purview and E5 suites, an edit is only checked for what it adds, and a licence that cannot be confirmed names the subscriptions ENCA read." },
      { kind: "fixed", tool: "Import", text: "📥 A policy Entra had created but was not yet showing is finished at the end of the run — checked again with a longer wait and completed as planned, including switching its old version Off — instead of being left as created but not verified." },
      { kind: "improved", tool: "What's new", text: "📋 From 2.0 the version reads v2.0.0, v2.0.1 …, the notes of the current major version stay open on this page, and all 1.x releases fold into the Archive below." },
      { kind: "improved", tool: "Help", text: "❓ Help documents every tool as it ships in 2.0, including new sections for ↗ Guided rollout and for the workspaces layout and navigation." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "MS Learn checks", text: "🧰 Fix now builds that second policy. It is a COMPANION, not an adjustment: the policy it came from is never modified, the scope is copied whole, the grant control is Require multifactor authentication with the authentication strength dropped, the session controls are left to the original, and it is born Off. It takes the next free CA number in the original's range rather than a bumped version — a bumped version reads as a replacement, and Housekeeping would treat it as superseding the original while Duplicates would offer to merge the two. A policy off the naming convention gets a name in words rather than a forced one. The Fixes tab marks it companion and says how many of the prepared policies go beside theirs instead of replacing them." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Home", text: "After sign-in the home page shows a band when the copy still signs in through an app another directory owns and you hold a role that can change that — Global, Privileged Role, Application or Cloud Application Administrator. Not now hides it for this tenant; the account menu keeps an Own registration… row." },
      { kind: "new", tool: "Sign-in", text: "The result is the default sign-in. For THIS BROWSER at once: the new registration is saved as a ⚙ connection and selected, and Default there restores the shipped one. For THIS DEPLOYMENT when an Azure Container App serves the page: one button writes ENCA_CLIENT_ID and ENCA_TENANT_ID onto the container app with your own Azure rights, the same way Branding settings writes ENCA_BRANDING — every visitor then signs in with your registration and it survives image updates. Any other host gets the js/authConfig.local.js block, the docker -e lines and the Azure template parameters to copy. Sign in again with your own registration closes the loop: the consent screen names your app, in your tenant." },
      { kind: "improved", tool: "Sign-in", text: "What the wizard needs from you is said before you press Create — consent, for you only, to Application.ReadWrite.All and DelegatedPermissionGrant.ReadWrite.All (and AppRoleAssignment.ReadWrite.All when assignment is ticked), asked on that click so a popup cannot be blocked mid-run — and so are the likely impact (nothing existing changes; the shared registration's consent stays until you revoke it) and the recovery (delete the registration, or ⚙ Forget the connection). Demo mode simulates every step and writes nothing." },
    ],
  },
  {
    build: 25437, date: "2026-09-22", title: "Policy builder — one guided screen for a policy",
    items: [
      { kind: "new", tool: "Policy building blocks", text: "\ud83c\udfd7 Policy builder (T41, BETA) — the sixth tab. Build a policy in seven guided steps: name and persona, who, what, when, then, session, state. The CA number is the next free one in the persona's range and the name follows the baseline convention as you go; every choice is a building block from the tabs beside it — the tenant's named locations, authentication contexts, strengths and terms of use are the pickers. Hints along the way are computed from the loaded policies: the break-glass group not excluded, legacy authentication already blocked tenant-wide, an IP location not marked trusted behind “All trusted locations”, an empty country location, an enabled policy that already targets the same people and resources." },
      { kind: "new", tool: "Policy building blocks", text: "Preflight before writing. The 🧪 Preflight view evaluates the draft with What-If against synthetic sign-ins — a member of what it includes, from outside and from inside a trusted location, a member of what it excludes, a B2B guest — alongside the loaded policies, so each row also says which other policies apply. The Card view is the policy card it will be, the JSON view the exact body that goes to Graph, and 📋 Change plan exports what the write will do as a Markdown file to hand to a reviewer first." },
      { kind: "new", tool: "Policy building blocks", text: "Born in report-only. Create writes the policy report-only, reads it back and opens it in 🗂 Policies; creating it On needs a typed ON in step 7. The policy's own exclusion group can be created first, from the baseline's naming rule, so the policy never names a group that failed to exist. Recovery is stated on the step: a created policy is restorable for 30 days from ♻ Deleted." },
      { kind: "new", tool: "Policy building blocks", text: "Three starts. ✨ Blank; 🧬 a baseline template — the tenant's own copy when it holds that CA number, otherwise the template's number, persona, groups and its own words; or 🗂 a policy from this tenant, to edit or to clone. Every block's detail pane has New policy with this location / strength / context / terms, which opens the builder with that step filled in." },
      { kind: "new", tool: "Policies", text: "\ud83c\udfd7 Open in builder on the action bar. One selected policy opens in the builder as an EDIT — a ± Diff view shows exactly what will change, and the PATCH sends whole sections (conditions, grant controls, session controls), only the ones that changed, so a clause nobody was looking at cannot be dropped — or as a CLONE, a new policy with the next free number in its range. With several selected the first one opens and the toast says so." },
      { kind: "improved", tool: "Baseline", text: "🧱 Update the catalog from this tenant: Select all and Deselect all above the list, and the two counts explained where they meet — the summary line counts versions in NAMES (“6 newer”), the panel compares DEFINITIONS (“26 changed”), and the panel now says how many of its changed policies also carry a newer version and how many changed with no version bump at all. Each changed card carries the same word: newer version in name, same version — edited in place, or older version in name." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Checks", text: "Five more, all from Microsoft's own support table for external users. Require approved client app, Require app protection policy and Require password change are not supported for guests at all \u2014 the first two need a device registered in your tenant, and a guest has no password here. Compliant device and hybrid joined device work only with inbound device trust. A user-risk policy blocks guests it can never remediate. And terms of use, sign-in frequency, persistent browser, app enforced restrictions and Conditional Access App Control are unsupported for B2B direct connect while working normally for collaboration guests. Three of these existed already but only for service provider (CSP) admins \u2014 the same policy breaking an ordinary B2B guest went unreported." },
      { kind: "new", tool: "Checks", text: "A matrix above the findings: the external user types your policies reach, against the controls those policies demand. Each cell reads ok, trust, blocked or n/a, and a control nothing asks of a type is left blank \u2014 nobody demands it is not the same as fine. Click a cell to open the policies behind it. Built from the policies already loaded and the cross-tenant settings the service provider checks already read, so it costs no extra call and no new permission; if those settings could not be read it says every trust answer is unverified instead of guessing." },
      { kind: "improved", tool: "Checks", text: "Local guest users are deliberately left out of all of it. That type is an account in your own directory with UserType Guest \u2014 your credentials, your management \u2014 so none of the cross-tenant limits apply to it, and reporting one as locked out would be wrong. An exclusion that names specific external tenants also does not take a guest type out of scope: it only reaches the tenants it names, and the findings say so." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Exclusion analyzer", text: "The same comparison answers the app question. An app excluded from an All-resources policy used to count as covered when some enabled policy carried any grant control and looked broad enough on its include side \u2014 its own exclusions were never subtracted, its platforms and controls never compared. A policy scoped to Windows, excluding the one user who mattered and requiring only a compliant device now reads PARTIAL with those three reasons named. Office 365 and Microsoft Admin Portals are resource collections and say so rather than being skipped without a word." },
      { kind: "improved", tool: "Gap analyse", text: "The coverage funnel says what it can prove. Two stages are renamed to TARGETED BY AN ENABLED POLICY and TARGETED BY AN ENABLED MFA POLICY: a policy demanding MFA of one app at high sign-in risk targets the user, which is not the same as MFA on every sign-in. Enforced is kept for What-If and sign-in evidence, and the users table column is MFA POLICY." },
      { kind: "fixed", tool: "Gap analyse", text: "Two policies with the SAME display name no longer share one matrix cell. Results were keyed by name, so an included policy and an excluded policy called the same thing collapsed into one entry and both columns showed the excluded state, while clicking either opened whichever policy came first. Everything is keyed by the immutable policy ID now, and a duplicated name carries a short ID in its column header." },
      { kind: "improved", tool: "Exclusion analyzer", text: "A verdict where a boolean used to be: the row subtitle counts how many policies have no equivalent coverage, how many are partial and how many could not be established, instead of one \u201cno other coverage\u201d number that covered all three." },
    ],
  },
  {
...
},
{
...
},
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "User or Group analyzer", text: "The sweep says its findings out loud instead of leaving them inside a count. Above the tiles: the ids a policy names that this directory does not have, which target nobody, and the groups nothing references at all \u2014 the first is a policy repair, the second is cleanup. A single-subject read that could not reach every service now says so in the same place, because nothing found where you were not allowed to look is not the same as nothing there, and the counts are a floor either way." },
      { kind: "improved", tool: "User or Group analyzer", text: "The per-area blocks \u2014 Entra ID, Intune, Microsoft 365, Azure \u2014 fold. Click a heading and it collapses, remembered per card, the same behaviour \ud83d\udd75 Who is and \ud83c\udf0a the wave have had since build 25323. On an account with hundreds of references that is a better answer than the jump links it needed to cope, and both are there." },
      { kind: "fixed", tool: "User or Group analyzer", text: "Its result strip was pinned at a hard-coded 106 pixels while every other sticky thing in the toolset reads the measured offset. Build 25353 gave this screen a toolbar, and the two then pinned to the same place: 62 pixels of overlap at a 1500 by 800 window, with the mode buttons and the exports hidden under the strip on any scroll. The strip pins where the result begins now \u2014 header, tab bar, host strip and this screen toolbar, all measured \u2014 and a jump lands its card clear of it." },
      { kind: "improved", tool: "All tools", text: "One implementation of the verdict tile, in js/coverage.js. The tile is the box that says one number and what it means, and it was the same three spans typed out by hand forty-six times across six files; \ud83e\udd25 Apps with no service principal had already written itself a private function for it, which is the tell. The new module also derives how many columns the row needs from how many tiles are in it \u2014 that was a class set by hand next to a count, and the two could disagree. \ud83d\udd17 is its first caller; the other six migrate next, with their output compared byte for byte." },
    ],
  },
  {
    build: 25355, date: "2026-09-14", title: "The tools inside a tool are the first thing you see",
    items: [
      { kind: "improved", tool: "All tools", text: "THE TAB STRIP IS ABOVE THE HEAD CARD NOW, ON ALL EIGHT HOSTS. It used to be the toolbar's first row, which put it UNDER the tool's description \u2014 and on \ud83d\udee1 Checks, whose head is four paragraphs, the four tabs sat most of a screen down. Somebody arriving there sees one tool, not four, and there is nothing on the way to tell them otherwise. The strip is navigation BETWEEN tools; the head describes the tool you are standing in, so navigation goes first. \ud83d\udee1 Checks (4), \ud83e\udde9 Policy building blocks (5), \ud83d\udea6 Sign-in log (3), \ud83d\udd75 Who is (3), \ud83d\udd53 Changes (2), \ud83d\udd2e What-If (2), \ud83e\uddec Baseline (2) and \ud83d\udd0d Gap analyse (2)." },
      { kind: "improved", tool: "All tools", text: "It stays pinned, because switching tool halfway down a result is the whole reason the strip exists. Two sticky rows given the same offset land on top of each other \u2014 which is exactly what hid \ud83c\udf0a's wave picker until build 25354 \u2014 so the strip pins under the tab bar and the screen's own toolbar pins beneath the strip, with the strip's height MEASURED into a variable rather than assumed, and re-measured by an observer when it changes. A screen with no strip is untouched: the variable reads zero there and its toolbar sits exactly where it did." },
      { kind: "fixed", tool: "All tools", text: "On a phone the strip was quietly EATING TABS. A segment control hides its overflow, so at 400 pixels three of \ud83e\udde9 Policy building blocks' five tabs sat past the right edge with no way to reach them, and the labels that did fit wrapped to three lines. The strip keeps its natural width and scrolls sideways instead \u2014 the same answer the policy list and the state chips already use there \u2014 so every tab is reachable at every width and the page itself never scrolls sideways." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Group usage", text: "\ud83d\udd17 User or Group analyzer has a toolbar. One group or user / Sweep every group is a scope and \ud83d\udd17 Sweep tenant with its three exports are actions, and both were sitting inside the form card \u2014 the mode at the top of it, the buttons at the bottom, neither where the same controls live on any other tool. They are a toolbar now, so this screen reads like the rest and can host a tab strip if it is ever folded into." },
      { kind: "fixed", tool: "All tools", text: "The last two toolbars with no id \u2014 \ud83d\udc65 CA groups and \ud83e\udd25 Apps with no service principal \u2014 have one. mountToolTabs finds a toolbar by id, so a toolbar without one can never host a tab strip, which made those two screens quietly ineligible as fold hosts; the filter row inside \ud83d\udd0d Gap analyse's panel got an id for the same reason. All 32 toolbars are addressable now, and tools/check-toolbar-order.js fails a commit that adds a toolbar without an id instead of merely noting it." },
      { kind: "improved", tool: "Protect exclusions", text: "\ud83d\udd12 Protect exclusions is the one tool screen with no toolbar, and it now says so on itself rather than looking like an oversight. Its controls belong to its result: the scan prompt and the per-group ticks are drawn by one renderer that serves this screen AND \ud83d\udc65 CA groups \u2465 from the same body element, so a toolbar here would have to exist on one of the two and not the other. The section carries data-no-toolbar with that reason, the checker reads it, and a screen that loses its toolbar without saying why fails." },
    ],
  },
  {
    build: 25352, date: "2026-09-14", title: "One tool, one title \u2014 and seven tools stop changing theirs",
    items: [
      { kind: "fixed", tool: "All tools", text: "SEVEN TOOLS WERE SAYING DIFFERENT THINGS ABOUT THEMSELVES DEPENDING ON WHAT THEY WERE DOING. Each tool screen opens with a line naming the tool, its chips and its number, and that line was typed into the tool's renderer once per state \u2014 reading, failed, loaded. So \ud83c\udfab Authentication contexts, \ud83d\udcaa Authentication strengths, \ud83c\udf10 Named locations, \u267b Recycle bin and \ud83d\udee1 Restricted AUs all DROPPED their writes-to-tenant chip while they were reading the tenant and got it back when they finished; \u26a1 CA validator's NEW chip did the same; \ud83d\udcdc Terms of use had three different title lines. A warning chip that disappears while a tool is working is worse than no chip: it is there while you read the result and gone while you decide whether to press the button. The chips describe the TOOL, not the state its head is in, so they are the same in every state now." },
      { kind: "improved", tool: "All tools", text: "The head line has one author. Each tool's title and chips are a field on its registry entry in js/version.js, beside its number and version, and toolHead is the only thing that writes the line \u2014 53 places used to. Two tools compose their own title because it is made of data rather than a name (\ud83e\uddec Baseline names its catalog and release, \ud83d\udc65 CA groups names the tenant); those keep composing it and take the chips and the number from the same place. A chip is named by its text and its colour comes from one table, so writes to tenant cannot be red on one screen and grey on the next." },
      { kind: "improved", tool: "All tools", text: "The version stamp is part of the line instead of something added afterwards. Until now a hand-kept map of 33 head element ids to tool ids sat in js/app.js with a MutationObserver on every one of them, re-appending the stamp each time a tool re-rendered its own head \u2014 33 observers and a list to keep in step with the tool list. Both are gone. The two heads written as static HTML, \ud83d\udd0d Gap analyse's intro and \ud83d\udd12 Protect exclusions, carry data-tool-head and are filled once at startup from the same registry entry, so their chips cannot drift from their tool either." },
    ],
  },
  {
    build: 25351, date: "2026-09-14", title: "Every tool screen is the same screen",
    items: [
      { kind: "improved", tool: "All tools", text: "THE CONTROLS ARE IN THE SAME PLACE ON EVERY TOOL. A toolbar holds six kinds of control \u2014 find, scope, filter, window, this tool's own extras, and the actions \u2014 and across the 33 tool screens there were TEN different orders for them. Scope came before filter on three screens and after it on seven; the range picker sat third on \ud83d\udd53 Change audit, \ud83d\udea6 Sign-in log and \ud83c\udf9a Report-only impact and fifth on \ud83d\udc65 CA groups. Nothing was wrong with any one screen \u2014 what was wrong is that a control moved when you switched tools, so you looked for it again every time. Every screen now runs find, scope, filter, window, extras, actions, in the markup as well as on the screen, so tabbing through a toolbar follows what you see. Eight screens had two controls swap places; no screen lost one." },
      { kind: "fixed", tool: "All tools", text: "One rule was behind most of the movement. The find box was flex:1, so it ate all the slack in the row: the filter chips landed in the middle of a screen that had a search and hard left on a screen that did not, which means a control's position depended on whether a DIFFERENT control existed. Inside a toolbar the find box is now a fixed width and the action group takes the slack \u2014 which is what it was already doing on the screens with no search. The find box is the same width in the same place on all seventeen screens that have one, at every window width." },
      { kind: "improved", tool: "All tools", text: "The spacing is in the stylesheet instead of typed in 96 times. A tool screen carries class screen tool and the stylesheet spaces its head card, its toolbar and its result; the 96 copies of margin-top:0 and margin-top:14px that used to do that job by hand are gone, so a new screen is spaced right by existing rather than by somebody remembering. The slot order is written down in css/app.css, and tools/check-toolbar-order.js fails a commit that writes a screen out of it \u2014 the same discipline as the plain-text check, and for the same reason: the stylesheet makes a wrong screen LOOK right, so only a checker can say it is wrong." },
      { kind: "fixed", tool: "All tools", text: "The stylesheet asked for Inter and the app has never shipped it, so every machine without Inter installed has been rendering ENCA in its own system font. The Content-Security-Policy is default-src self and nothing here loads from a third-party CDN at runtime, so the name is simply gone from the stack: what is declared is now what renders, on every machine." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "All tools", text: "There is ONE scope check now — js/cascope.js, CaScope.of(policy, subject) — answering the question every tool has to answer before it can say anything about a person and a policy: included, excluded (included by one clause and taken out by another, which is the interesting one because it is a bypass), or not addressed at all. It is built from 🕵's implementation, which was the more complete of the two, and ⚡'s appliesTo and 🕵's stateFor both delegate to it keeping their own field names, so no call site moved. Two implementations of one question is how a tool ends up quietly answering it wrong." },
      { kind: "fixed", tool: "CA validator", text: "A second disagreement the shared check settles, and it changes one word on screen. The old scope check looked for an exclusion INDEPENDENTLY of whether the target was included at all, so a break-glass account that sits in CA200's exclusion group but is not in CA200's Internals scope was reported as EXCLUDED from a policy that never addressed it. It now reads NOT TARGETED, which is what 🕵 Who is Anna to CA has always said about the same person. The verdict itself never differed — checked over 120 policy-by-subject comparisons on the demo tenant, every one of which agrees on whether the policy applies; only the reason for an out-of-scope row changes, and only for a target that is in an exclusion group without being in the include scope." },
      { kind: "improved", tool: "All tools", text: "Why the guest type arrives two ways, now handled in one place: a tenant reports it either as its own includeGuestsOrExternalUsers boolean or as a GuestsOrExternalUsers token inside the user list, and the token has to be kept OUT of the id comparison or it is matched against object ids and never fires. Both readings are in CaScope.prep, so every tool gets both." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "fixed", tool: "Who is the wave to CA", text: "THE FLAGS COLUMN COULD NOT BE READ. Each flag rendered its whole list into a single span joined with commas, and that span carries white-space nowrap, so it could not break anywhere: a member in five exclusion groups forced the column past the card, the table began scrolling sideways, and the sentence ended at CA114 exclu. Nothing was missing from the data — it was simply unreadable, which on a 500-member wave is the same thing. It is a chip per policy now, in a cell that wraps: the column grows DOWN and can never push the table wide again." },
      { kind: "improved", tool: "Who is the wave to CA", text: "What the chips say changed with the shape. The word exclusion came off every chip and onto one count line underneath — five chips said it five times — the chip's COLOUR carries the state instead of a parenthetical, red for a live bypass and plain for an exclusion on a policy that is not On yet, and the full sentence is on each chip as a tooltip. The count line spells out the split — 3 exclusions — 1 On, 2 not On — and appears only when there is more than one chip, because a lone count line under a single red chip restates the chip and wraps to three lines in a column that narrow. Same change in the two-waves flag." },
      { kind: "new", tool: "Who is the wave to CA", text: "👥 Members takes the whole window on a button, because 500 members is five columns of them. It is a state flag rather than the shared full-screen modal: the wave rewrites its body whole on every filter click and every risk read, and an element parked in that modal would not survive the first re-render. Folding the card no longer hides it in full screen, and Esc puts it back." },
      { kind: "improved", tool: "Demo tenant", text: "The demo can show the Flags column now. No demo member was in more than one exclusion group, so the very thing this build fixes was invisible on ?demo=1 — CA212 and CA310 have their own exclusion groups, and Eva is in three groups across an enforced and a report-only policy, which is the mix the chip colours and the count line exist to tell apart." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Sign-in log", text: "Two things called report-only on one tool would have been the confusion this fold created, so 🚦's own Report-only mode now says what it is: the individual sign-ins a staged policy WOULD have failed, newest first — with a link to the 🎚 Report-only impact tab, which answers the different question (is this one policy safe to enable?) from the same window." },
      { kind: "improved", tool: "Session controls", text: "Beta-only is a guarded TAB now, not a deleted file. The tab is hidden on the production host by the same isProdHost test the BETA ribbon uses, so the carve-out in the promotion port stops being 'delete sessionctl.js and its tile' and becomes one guard that is visible in the code." },
      { kind: "improved", tool: "All tools", text: "A folded tool now opens where it actually lives. ⌘K for 'report-only' or 'session' offers the tool with its own name and lands on its TAB, not merely on the host, and the same is true of every in-app link written against the old tile — including 🧩 Baseline (Joey Verlinden), which from now on opens the Baseline tool ON his catalog rather than on CloudFellows. T26 and T38 keep their numbers and their Help sections, listed in 🔢 Tool numbers with the tab that holds them." },
      { kind: "improved", tool: "Roadmap", text: "R53 🚦 One tool per data source — phase 2 of the consolidation review, of which this is the first of four hosts. 🕓 Changes (audit log and snapshot behind one source switch), 🧩 Policy building blocks (locations, strengths, contexts, terms of use, deleted) and 🛡 Checks (bypass patterns, Microsoft Learn, CIS, Intune reality as rule packs over one findings table) follow." },
    ],
  },
  {
    build: 25338, date: "2026-09-11", title: "Four tiles fewer, nothing removed: one door per room",
    items: [
      { kind: "improved", tool: "Policies", text: "ONE DOOR. Four tiles opened this one screen: 🗂 List Policies, 📄 Create documentation, 🗄 Backup (JSON) and 🎚 Set Policy state. The last three set a mode and then showed a toast naming a button that was already on the action bar — a second door onto a room that had one. They are one tile now, called 🗂 Policies, and its blurb names the five verbs the bar offers: Create documentation, Backup, Gap analyse, Assign groups or roles, Set policy state. The bar, the per-card quick actions and every deep link from another tool are exactly as they were; the one change is that no action arrives pre-highlighted, because there is no longer a tool you came from — the highlight now means the action you last ran here." },
      { kind: "improved", tool: "Baseline", text: "One tile, both catalogs. 🧩 Baseline (Joey Verlinden) was a second tile calling openBaseline for the same screen the CATALOG PICKER in the toolbar has switched between since 2.0 — so the tile was a shortcut to a control one click away. The tile is 🧬 Baseline, the picker is the way between CloudFellows R26.6 and Joey Verlinden, and opening the tool now lands on the tenant's ★ ACTIVE catalog instead of always CloudFellows. Nothing about his catalog changed: the live repository read, the bundled 2026.6.1 fallback, the naming contract and the ★ switch all still live on its own summary card." },
      { kind: "improved", tool: "All tools", text: "The folded numbers are PUBLISHED, not just kept. T02, T04, T05 and T11 keep their T-numbers — the rule has always been that a number is never reused, not when a tool is retired and not when it is folded — and a number nobody can look up is worth nothing, so ❓ Help 🔢 Tool numbers now lists each folded number with the tool that hosts it. Each of the four keeps its own Help section too, moved under its host as a sub-section with its emoji and name intact, and those sub-sections are in the Help contents strip, indented. Any in-app link written against a folded tile still lands on the host, and the ⌘K COMMAND PALETTE still answers to the old name: typing documentation, backup or Joey offers the folded tool and says which tool it lives in now and where inside it — a fold that takes a word out of the search box has hidden the verb, whatever the Help says." },
      { kind: "improved", tool: "Roadmap", text: "R52 🗂 One door per room — phase 1 of the tool consolidation review: 39 tiles to 35. The review found five tiles opening the same screen in different modes, two calling one function with a different argument, four carrying a private copy of the same helper, and three reading the same sign-in window twice. Phase 2 is the tabbed hosts — 🚦 Sign-in log, 🕓 Changes, 🧩 Policy building blocks, 🛡 Checks — and is not in this build." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Who is the wave to CA", text: "The same check across a wave: how many members satisfy a reaching approved-client-app policy through the app, how many of those already show an app protection policy working, and the red rows — the members who are blocked the day the control goes unless an APP policy reaches them first. Three tiles, the policies with their reach, the member table, every name opening 🕵; in the export." },
      { kind: "improved", tool: "Report-only impact", text: "A staged policy that grants on Require app protection policy now says under its verdict that report-only cannot evaluate that control — Microsoft documents it reports Failure there even where it passes once enforced — so its would-deny count is not evidence; enable it for a pilot group to see the real answer." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Who is Anna to CA", text: "Every card — deployment groups, policies, identity risk, devices, MFA, stopped sign-ins, forecast — folds on its heading. Click to fold or unfold; the choice is remembered per card, so the next user or a rescan keeps the layout. A folded card keeps its heading and the counts in it." },
      { kind: "improved", tool: "Who is the wave to CA", text: "The same folding on the wave's cards — identity risk, readiness, policies, members, how the wave is built." },
      { kind: "improved", tool: "Who is Anna to CA", text: "The line under the Identity risk card says in plain words where each part comes from — the user-risk line and detections from Identity Protection (filled only on Entra ID P2), the risky sign-ins from her own records — and that the card only reads: dismissing or confirming a risk is done in the Entra portal under Identity Protection → Risky users." },
    ],
  },
  {
    build: 25322, date: "2026-09-10", title: "Who is Anna answers the MFA complaint; the wave reads identity risk",
    items: [
      { kind: "new", tool: "Who is Anna to CA", text: "🔐 MFA on her sign-ins. The complaint is always the same — “he keeps getting the Authenticator prompt on app X, which policy is it?” — and the stopped-sign-ins table could never answer it, because a prompt the user completes is a successful sign-in. The new card can: per app, the applied policies that demanded MFA on her sign-ins, how many sign-ins required it, and from the record's own authentication steps how many were a FRESH prompt versus a claim already in the token, from how many devices, with the last fresh prompt. Then it says why: fresh prompts spread over many devices (a non-persistent VDI pool is one prompt per host — the Perfetti case, nineteen AVD hosts in a week), a sign-in frequency or non-persistent browser session reaching her, or every requirement satisfied by the token — in which case the prompt she sees is per-user MFA, security defaults or the app, not Conditional Access. Hunting rows carry the requirement but not the steps and are counted as not known; the Entra log source tells them apart. In the Markdown export too." },
      { kind: "new", tool: "Who is the wave to CA", text: "🛡 Read identity risk, on demand, for the whole wave: Identity Protection's user risk per member (one read each, twenty to a batch; needs IdentityRiskyUser.Read.All and Entra ID P2), joined to the risky sign-ins the members already have from the shared window and to the risk-based policies aimed at the wave. Three tiles — members at risk by level, members with a risky sign-in in the window, risk policies firing now on whom — a list of the risk policies with how many members each reaches, and a table of the members that are flagged, remediated or had a risky sign-in, each opening in 🕵 Who is Anna. Before the read, a callout already names the members with a risky sign-in in the window. In the Markdown export." },
      { kind: "improved", tool: "Sign-in failures", text: "The hunting read now also carries AuthenticationRequirement, RiskLevelAggregated and RiskState, so the two cards above have the same inputs from Defender hunting as from the Entra log — bar the per-step authentication detail, which hunting does not have." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Session controls", text: "The same ■ Stop button. Stopped during the Defender hunt, the slices already read are kept and rendered as a PARTIAL window — the note names how many of the slices were read and that the sign-in window was skipped — because the events so far are real events and a large tenant's 30 days is dozens of two-minute queries. Stopped during the sign-in read, the Defender half stays and the note says routing to a CA policy is missing. A stop while another tool's read is being joined leaves the join and lets that read finish for its own tool." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Report-only impact", text: "Reads from the shared sign-in source and says which one in the header — and says when non-interactive sign-ins are in the verdicts, because a report-only forecast that counts token refreshes is a different number." },
      { kind: "improved", tool: "Session controls", text: "The four tiles are filters: the number sets the table it counts and the counts under each tile narrow further — App Control / other session controls, policies that routed a session, logins logged, blocked, protected, step-ups, audited, unmatched — and the page scrolls to the table the tile filtered. A Routed-a-session policy chip was added to match." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "Baseline Policies", text: "The count chips on the summary card (12 missing, 26 present, 6 not in baseline) are the filter buttons — click one to see those rows, click it again for all — and they stay in step with the chip row under the toolbar. An active baseline with missing policies opens on Missing, because what is still to do is the question the screen answers." },
      { kind: "improved", tool: "Baseline Policies", text: "E-Admins on every catalog. Emergency access is not one baseline's idea, and only the CloudFellows catalog writes those six policies out (CA1100 to CA1105), so a tenant on Joey's baseline that had them showed six not-in-baseline rows for doing the right thing. They are now expected under every catalog: his table carries them as a 🚨 E-Admins section marked shared, coverage counts them, and the import line says they come from the CloudFellows backup rather than his repository. The leftovers scan already treated them as both baselines' and needed no change." },
      { kind: "new", tool: "Baseline (Joey Verlinden)", text: "📥 Import baseline needs no zip. His repository is laid out exactly like a backup — a policy file per policy, Config/Groups with one JSON per group carrying the ids the policies reference, Config/NamedLocations — so the live read now lists the whole Config folder in one tree call, reads the group and named-location files alongside the policies, keeps the raw objects with the OData noise stripped (a 16 KB export becomes 5 KB in the session cache), and the button hands that read to the Import tool with the gap ticked and the rest of the release listed unticked. A read whose folder listing was refused says an import would drop every group reference and asks for a re-read first; a catalog without a repository still asks for the zip." },
      { kind: "new", tool: "Import", text: "🔀 Switch baseline, a third assignment mode next to Deployment groups and Match and replace. Policies land with the baseline's own groups as shipped — created here, so his \"<policy name> - Exclude\" and CA-BreakGlassAccounts - Exclude, or the CAB-SEC-U-CAnnn-Exclusion set the other way — and the members of the other baseline's counterpart groups are COPIED across before the policies land: break-glass to break-glass, the exclusion group of the same CA number (the convention's name for it even when the catalog lists none, because a tenant added CAB-SEC-U-CA000-Exclusion in the field), the persona include group. A copy, through the same helper ⑦ Migrate uses: the old group keeps its members as the rollback and 🧹 What the previous baseline left behind lists it once nothing references it. A superseded same-policy-other-name is switched Off. The mode is offered when the file matches a baseline the tenant is not on, pre-selected when this tenant's policies visibly exclude the other baseline's groups, and the report lists every copy with its count, every counterpart that did not exist, and flags the include group his baseline ships as an example." },
      { kind: "fixed", tool: "Import", text: "The plan understands a baseline that does not version its names. It required a vX.Y on both sides to call a policy present, so every one of Joey's 38 was offered as an import even when the tenant had it under the exact name. Same CA number plus the same name (staging prefix aside) is present now; a same-number policy whose name contradicts — the CloudFellows CA000 next to his CA000 — is never treated as the same policy at another version and replaced in place, it is deployed alongside. His Agents persona is recognised and its include stays as shipped rather than falling into the guest-admins range, and a persona with no deploy group no longer asks to create a group called undefined." },
    ],
  },
  {
    build: 25248, date: "2026-09-02", title: "Teams devices: the shared-device group checked against what the tenant actually licenses",
    items: [
      { kind: "new", tool: "Teams devices", text: "New tool (T35, BETA). The baseline keeps Teams Rooms, panels, common-area phones and call-queue resource accounts out of MFA, sign-in frequency, device-code blocks and risk policies by excluding ONE dynamic group — CAB-SEC-U-TeamsSharedDevices — and the rule that group has carried names three Teams ROOMS service plans and nothing else. A tenant whose phones sit on Teams Shared Space (Teams Shared Devices until April 2026) and whose call queues use Teams Phone Resource Account licences had none of those accounts in the group. The tool reads the tenant's subscribed SKUs, keeps the service plans that exist ONLY in device licences (a rule cannot name MCOEV or TEAMS1 without pulling every E5 user in), builds the membership rule from those, previews how many accounts it matches today with the first few by name, grades every group that looks like a Teams group — current, update the rule, matches PEOPLE, assigned — and lists the active policies that still reach the device accounts with a control from Microsoft's not-supported table. Replace the rule writes it onto the group after a confirmation showing old and new side by side; Create makes the group when a tenant has none. UPN prefixes typed in the toolbar cover accounts no service plan can isolate. Teams Phone Standard is a user licence and stays out on purpose." },
      { kind: "improved", tool: "Conditional Access groups", text: "The bundled CAB-SEC-U-TeamsSharedDevices template now ships the full device-plan rule — Teams Rooms Basic, Pro, Standard and Premium, Teams Rooms Pro Management, Teams Shared Space (Specialty devices) and Teams Phone Resource Account — so a group created from ② Create starts with every device population, not only the rooms. A group created from the old template keeps its old rule until the Teams devices tool replaces it." },
      { kind: "fixed", tool: "Layout", text: "The footer bar sits on the bottom edge of the window on a short page — the sign-in screen, a tool before its first run, an empty result — instead of floating mid-page with empty background under it. A long page scrolls as before and the bar follows the content." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Baseline (Joey Verlinden)", text: "Read LIVE from github.com/j0eyv/ConditionalAccessBaseline. Once per session the tool reads the latest release, its Config/ConditionalAccess listing and every policy JSON, and the card says which release and commit it read and how long ago. The hand-checked snapshot (2026.6.1 at 38469a4) stays as the fallback, and a read that fails — GitHub's unauthenticated rate limit, a moved folder, a malformed release — leaves it in place and states the reason on the card rather than falling back quietly. Every file is validated before it is believed (JSON, an object, a CAnnn- display name, a bounded size and count; fewer than ten usable policies means the release is treated as malformed), nothing from it is executed, and a CA number used by more than one file is shown as such — the first live read found 38 files at the 2026.6.1 tag where the snapshot lists 36, two of them second copies of CA005 and CA006." },
      { kind: "new", tool: "Baseline (Joey Verlinden)", text: "The naming contract. His exclusion groups are named after the POLICY (“<policy name> - Exclude”), not the CA number, so his baseline routes a group to a persona by an EXACT, case-insensitive match against a policy name the catalog knows — and by nothing else. A group that matches nothing stays visible as unmapped, and the tenant's own R28 mapping can place it. His personas — Global, Admins, Internals, ServiceAccounts, Guests, Agents — are their own vault list rather than a rename of ours: a code is shared with a CloudFellows persona only where it means the same thing (his CA300 block is service accounts, not externals), and AGENTS is his alone." },
      { kind: "improved", tool: "Conditional Access groups", text: "① Check and ② Create work against the active baseline. With Joey Verlinden's active, the expected set is his 36 “- Exclude” groups plus CA-BreakGlassAccounts - Exclude and CA-ServiceAccounts, all creatable as plain security groups from templates derived from the catalog — which means the live read's fresh names flow through without a code change. The summary carries a “Working against …” chip, and hints when three or more of the tenant's policies carry names from the OTHER baseline. The union of both catalogs that the scan used to expect is gone: a missing list nobody had chosen between was noise, not a finding." },
      { kind: "improved", tool: "Restricted AUs", text: "The persona units follow the active baseline. For Joey Verlinden's that is one unit per persona he defines plus break-glass — CA-RMAU-Global-Exclusions, CA-RMAU-Admins-Exclusions, CA-RMAU-Internals-Exclusions, CA-RMAU-ServiceAccounts-Exclusions, CA-RMAU-Guests-Exclusions, CA-RMAU-Agents-Exclusions and CA-RMAU-BreakGlass — ENCA's convention under his prefix, because his baseline defines no administrative units, and the panel says so. ＋ Bulk add and the persona chips scan his group family (the CA prefix) instead of CAB-SEC / CAD-SEC, and 🏷 Group personas is kept per baseline: a mapping to Externals saved under CloudFellows is not lost when Joey's is active, it is simply not in that drawer." },
      { kind: "improved", tool: "Protect exclusions", text: "⑥ Protect routes a group into the active baseline's vault — through the same one function as before (the R28 mapping first, the baseline's rule second, unmapped left visible) — and the fallback unit it offers to create is named after that baseline's prefix." },
      { kind: "improved", tool: "List Policies", text: "The exclusion-restore action (👥 Assign, action 8) names the active baseline's group for each policy: CAB-SEC-U-CA101-Exclusion under CloudFellows, “<policy name> - Exclude” under Joey Verlinden's, with the same catalog / derived labelling. The pick-by-persona chips follow it too; his internals include group ships as an example (APP_Microsoft365_E5) and the chip says so." },
      { kind: "improved", tool: "Baseline guide", text: "The readiness checks read the active baseline's groups, units and policies. The step prose was written for the CloudFellows deployment, and the header says so — in capitals, not in a footnote — when the other baseline is active, because his persona model and ranges differ." },
      { kind: "improved", tool: "MS Learn checks", text: "The break-glass group the fix creates when the tenant has none is the active baseline's (CAB-SEC-U-BreakGlass, or CA-BreakGlassAccounts - Exclude); the other stays an accepted alias, because a tenant that switched still has the group it made under the previous name." },
      { kind: "improved", tool: "All tools", text: "The content-security policy names two more hosts, api.github.com and raw.githubusercontent.com, for the live read and nothing else — written down beside the meta tag, as the R39 card said any widening should be. The self-hosted image inherits it." },
      { kind: "fixed", tool: "Baseline (Joey Verlinden)", text: "The table header and the Import hand-off both printed the CloudFellows release designation (R26.6) whichever catalog was on screen. They now print the catalog being compared." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Conditional Access groups", text: "A group added by hand is treated exactly like a scanned one rather than trusted because somebody typed it. ⑥ Protect reads its administrative-unit membership and routes it to its persona vault; ⑦ Migrate reads the directory roles it holds, counts the members that would move, and checks whether it already sits in a restricted unit — the same three reads a scanned candidate gets, because the plan has to rest on the tenant rather than on a name. It is pre-selected, marked added by hand with an ✕ to take it back off, and it SURVIVES a rescan: a rescan is a fresh reading of the tenant, not a reason to forget what you put on the list." },
      { kind: "improved", tool: "Conditional Access groups", text: "The ＋ Add button refuses before the write instead of after it. ⑦ Migrate will not take a group that is not role-assignable — there is nothing to convert, and ⑥ Protect is where it belongs — and ⑥ Protect will not take what the directory rejects as an administrative-unit member: a Microsoft 365 group, a mail-enabled security group, a distribution list or an on-premises-synced group. Each refusal names the way out rather than stopping at the word no, and the same guard now runs at the write itself, so a selection that survived a rescan cannot reach Graph and come back as an error nobody can read." },
      { kind: "fixed", tool: "Conditional Access groups", text: "In ⑥ Protect, an exclusion group no policy references had a DISABLED checkbox while the sentence printed under it said tick it deliberately if you want it protected. The row was telling you to do something the row would not let you do — and it made every hand-added group unusable, since a group nothing references is exactly what somebody searches for. It can be ticked now. It is still left out of the pre-selection and out of ☑ Select all, which is what that rule was actually for, and the counter beside the button now says which is which instead of calling all of them cannot be added." },
      { kind: "improved", tool: "Conditional Access groups", text: "Clicking a group on ① Check opens a panel that answers where this group is used — and then made you go and find each of those policies by hand. The policy names under Included in and Excluded from are links now: click one and its policy card opens, the same as every other policy list in the app. A name with no policy loaded behind it stays plain text and says so, rather than looking like a link that does nothing." },
    ],
  },
  {
    build: 25238, date: "2026-08-28", title: "The page has a green under it again",
    items: [
      { kind: "improved", tool: "Self-hosting", text: "SELF-HOSTING.md gains the step before every other step: installing Docker. It led with a curl one-liner that only works on a machine already set up for it, which is the wrong assumption for a document written for somebody standing up their first instance. Now there is a prerequisite section with the command per platform - brew install --cask docker-desktop, winget install -e --id Docker.DockerDesktop, and the Docker Engine guide for Linux with the usermod line that stops every command needing sudo - plus direct installer links for each architecture and a one-liner that answers whether Docker is actually ready." },
      { kind: "fixed", tool: "Self-hosting", text: "The Homebrew cask was renamed: it is docker-desktop now, and docker is the old token. Any note still saying brew install --cask docker is out of date, so the document says so rather than leaving somebody to work out why the command they were given does not do what they expect." },
      { kind: "fixed", tool: "Self-hosting", text: "Every multi-line command in the self-hosting documentation was bash, pasted by people on Windows. A backslash continues a line in bash and does nothing in PowerShell, so the first line ran alone and docker answered \"invalid reference format\" - an error naming neither the real problem nor the fix, followed by PowerShell complaining that -e is not a cmdlet. The docker run and the Azure restart now carry a PowerShell form alongside the bash one, with the backtick continuation and $() replaced by a variable, because the substitution syntax differs too and a half-translated command is worse than none." },
      { kind: "fixed", tool: "All tools", text: "The update window inside the app had the same fault and now writes its Azure block for the platform the browser is running on. The branding gear's Copy for container instructions lost their line continuation entirely and are one line: a backslash is wrong for PowerShell, a backtick is wrong for bash, and there is no wrapped form that is right for both." },
      { kind: "improved", tool: "Self-hosting", text: "The install scripts say the same thing at the moment it matters. Not finding Docker was answered with a download page to go and read; each script now prints the command for the platform it is running on, detected rather than guessed. install.ps1 also warns that Docker Desktop pulls in WSL 2 and that this needs a REBOOT - better known before starting than discovered halfway through - and both scripts now name how to start a daemon that is installed but down, which is the commonest way the quick start fails." },
      { kind: "improved", tool: "All tools", text: "The light theme's page background moves from a near-white with a hint of green to a proper mint, and the white cards now sit on something rather than fading into it. The old value was chosen to be quiet and ended up being nothing: on a bright screen it read as white, so a page of white tiles on a white page had no depth and the eye had nothing to tell a card edge from the canvas. One variable changed, the page background; every surface, border and text colour is untouched, so nothing that was legible before has moved." },
      { kind: "improved", tool: "All tools", text: "Dark mode is deliberately unchanged. Its background was never the one that read as flat, and moving both at once would have made a look that needed judging in two themes rather than one." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Self-hosting", text: "On a Container Apps host the dialog gains a Branding JSON URL box. Fill it in and Save to this deployment writes ENCA_BRANDING_URL instead of the inline value - no size limit whatever, since the container fetches the file on every start - and clears the other variable, so there are never two sources of truth for one look. Leave it empty and the look is saved inline as before. A URL that is not plain https is refused in the dialog, before anything reaches Azure." },
      { kind: "fixed", tool: "Self-hosting", text: "Giving that path a text box meant tightening what the branding sanitiser accepts as an asset. It tested for the absence of a colon, which cannot see two things: a protocol-relative //evil.example/x.png, which is absolutely off-origin, and .. traversal. The Content-Security-Policy's img-src 'self' refuses the first in a browser, and that is defence in depth rather than a reason to accept the value - a branding names an asset ITS OWN deployment serves, and anything else is now rejected outright. The hole was reachable before this build through Import from JSON; a box to type in would have made it easy." },
      { kind: "improved", tool: "Self-hosting", text: "The path boxes open EMPTY on a deployment that has not set one, rather than pre-filled with the app's own logo path. Pre-filling would have looked helpful and quietly pinned every saved branding to one build's ?v= asset URL, leaving it pointing at a stale file some builds later." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "All tools", text: "Widening connect-src is not a thing to do quietly, so it is written down where it happened: the meta tag now carries a comment saying which host was added, why the app has always intended to reach it, and what the silent failure looked like. One host, named, reviewable in the diff." },
      { kind: "new", tool: "Self-hosting", text: "The branding gear can now save to the deployment itself. On an Azure Container Apps host a Save to this deployment button writes the look onto the container app's own ENCA_BRANDING setting - so it applies to EVERY visitor and survives restarts and image updates, because the setting lives on the Azure resource and not inside the container. It closes the loop that used to run: design the look, copy the value, leave the app, find the resource in the portal, paste it in." },
      { kind: "improved", tool: "Self-hosting", text: "This is the only place ENCA writes to Azure, and it is bounded on purpose. It changes one environment variable on the container app serving the page you are looking at, using the signed-in user's own token, with ARM consent asked on that click rather than at sign-in - a permission requested at the moment it is used is one somebody can actually judge. Nothing is stored and nothing is delegated: a user without rights on that resource gets Azure's own 403 and a message saying so. The container app is found by asking Resource Graph which app carries this page's ingress hostname, because the browser knows its own hostname and nothing else - not the subscription, not the resource group - and making somebody paste a resource id into a form is the friction this removes. The whole template is read and sent back rather than the env list alone, since a container-app patch replaces the containers wholesale and a narrower one would have dropped ENCA_CLIENT_ID and quietly moved the deployment back to the shared registration." },
      { kind: "improved", tool: "Self-hosting", text: "Two corrections that build 25232 described a build before they existed. The update window's Azure block leads with restarting the active revision rather than copying it - Container Apps pulls the image whenever a container starts, so a restart is the lightest thing that re-pulls - and it warns off az containerapp update with an unchanged image tag, which is not a revision-scope change and can do nothing at all. And the entrypoint no longer dies when ENCA_BRANDING is set on a deployment that also mounts selfhost-branding.json read-only." },
    ],
  },
  {
    build: 25233, date: "2026-08-28", title: "A setting nobody can see in the portal now says who set it",
    items: [
      { kind: "new", tool: "SMS & voice retirement", text: "Passkey dynamic migration answers the question that always follows the value: since when, and who did that? A 🕓 Who changed this? button beside 🔎 Check reads the directory audit log for edits to the authentication methods policy and writes the answer under the state strip - the transition in the strip's own words, not paused → paused, the person or app who made it, their IP address and the time, with every policy edit in the window behind a Show all. There is nowhere else to look this up: the property has no control in the Entra admin center, so it has no change record anybody can reach by clicking." },
      { kind: "new", tool: "SMS & voice retirement", text: "Three answers, and they are deliberately not written as one. Either the audit log recorded the property moving, and the name against it is the name that moved it; or the policy was edited without Entra diffing the nested property, which is said as exactly that, because putting a person's name against a change they may not have made is worse than admitting the log is unclear; or there is nothing in the window, which is reported as the WINDOW rather than as history - audit retention is licence-bound at roughly 30 days on P1 and P2, seven days otherwise, so no record means not in the window, never never happened." },
      { kind: "improved", tool: "SMS & voice retirement", text: "Its own button and its own scope, asked for on that click. AuditLog.Read.All is not needed to read or to pause the rollout, so a tenant that refuses it still gets the value - it just does not get the name, and the strip above is left exactly as it was rather than turned red by a failure that has nothing to do with what the setting says. The demo tenant carries demo audit records, parsed by the same code the real path uses, and a demo pause joins them so the history never contradicts the strip it belongs to." },
    ],
  },
  {
    ...
    items: [
      ...
      { kind: "improved", tool: "All tools", text: "The update window now leads with the command for the platform it is running on. On an azurecontainerapps.io host that is a restart of the active revision, not a docker pull there is nowhere to run; docker and the reviewed-fork route follow. Container Apps sets every container's image pull policy to always, so anything that starts a fresh container re-pulls the tag - a restart, a deactivate and activate, a new revision, or a scale-to-zero cold start - and restarting is the lightest of those. One command is deliberately NOT offered: az containerapp update with the same image tag, because an unchanged image reference is not a revision-scope change and can do nothing at all, which is worse than no command." },
      { kind: "improved", tool: "All tools", text: "Every block in that window has a Copy button, and where the clipboard is blocked - a plain http origin, or a denied permission - it selects the text and says to press Ctrl+C rather than being a button that silently does nothing." },
      { kind: "fixed", tool: "Self-hosting", text: "Setting ENCA_BRANDING on a deployment that ALSO mounts selfhost-branding.json read-only - which is what install.sh and the compose file do - killed the container. The write failed on the read-only file and took the whole entrypoint down with it, turning a merely redundant configuration into an instance that never starts. It now keeps the mounted file, says which of the two is in force, and carries on. The equivalent failure on js/authConfig.js stays fatal on purpose and now explains itself: carrying on there would serve the image's built-in registration while the operator believes their own is in use, and people would consent to the wrong application. A container that refuses to start is a problem you can see." },
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Self-hosting", text: "The container now takes ENCA_CLIENT_ID, ENCA_TENANT_ID and ENCA_AUTHORITY. An entrypoint applies them at start, and does nothing whatever when they are unset, so every existing deployment is unaffected. It writes above js/authConfig.js rather than editing values inside it, using the Object.assign hook that file already ended with - so nothing has to match a pattern in the app's own source, and a refactor upstream cannot silently break the injection and leave a container quietly signing in to the wrong registration. Idempotent, because a restarted container keeps its writable layer and would otherwise stack a block on every start." },
      { kind: "improved", tool: "Self-hosting", text: "Values that are not a plain guid or https URL are REFUSED, and the container fails to start naming the variable, rather than being escaped and served. An environment variable becomes a string literal in a script that every visitor to the sign-in page runs, so a value able to close that literal is script injection - and there is no legitimate client id the check rejects. Reachable everywhere the package goes: the clientId and tenantId parameters on the Deploy to Azure template, environment keys in docker-compose.yml, ENCA_CLIENT_ID for install.sh and -ClientId/-TenantId for install.ps1." },
      { kind: "fixed", tool: "All tools", text: "New-EncaAppRegistration.ps1 -SingleTenant patched js/authConfig.js with the new client id, which is the canonical app's identity, tracked in git and served from the canonical host. It printed the js/authConfig.local.js instructions and then overwrote the shared file anyway - leaving a working tree one git commit -a away from pointing the published site at a private tenant. It now leaves that file alone on a single-tenant run and says so, and points at the container variables as the route that needs no editing at all." },
      { kind: "fixed", tool: "All tools", text: "The same patch was also rewriting the file's own documentation. Its pattern matched any clientId followed by a quoted string, and js/authConfig.js opens with a header comment showing the example line clientId: \"<your Application (client) ID>\" - so every run replaced the placeholder in the instructions with a real tenant's id, teaching the next reader that a specific customer's guid is what belongs there. The pattern is now anchored to the assignment's own indentation, and a run that cannot find it says so and changes nothing rather than guessing." },
    ],
  },
  {
    build: 25229, date: "2026-08-28", title: "A self-hosted copy says so",
    items: [
      { kind: "fixed", tool: "All tools", text: "A self-hosted instance called itself BETA, in red, at the top of every page. It was not beta - it was somebody's own deployment, stood up on purpose on their own infrastructure - and the app had no way to tell the two apart, because it only knew production from not-production. An organisation told every day that the tool it deployed is a test build learns to ignore the ribbon, which is the one thing a ribbon exists to prevent." },
      { kind: "improved", tool: "All tools", text: "There are three deployments now, not two. Production is the canonical host. The publisher's own pre-production site is named in branding as betaHost and still wears the red BETA ribbon. Everything else - a container on localhost, an Azure Container App, a fork on its own domain - wears a slate SELF-HOSTED one and a [SELF-HOSTED] title tag, whether or not a selfhost-branding.json is served. localhost counts as self-hosted deliberately: docker run with a published port IS the documented quick start, and the build stamp on the sign-in card is what tells a developer which build they are looking at. Neither host name is reachable from the branding file, because a copy that could name itself the publisher's host could hide that it is a copy." },
      { kind: "new", tool: "All tools", text: "The sign-in screen says it too, and says more, because a ribbon is a glance and signing in is the moment that matters. On a self-hosted host the card names where it is served from, the build it is running, that it does not update itself, and - the part that decides whether signing in is safe - the full client ID of the app registration about to receive the consent, with whether its authority is the shared multi-tenant one or a single tenant. The client ID is shown whole and selectable: an identifier truncated for tidiness cannot be checked against your own tenant, which is worse than not showing it, because it looks like verification." },
      { kind: "improved", tool: "All tools", text: "New-EncaAppRegistration.ps1 documents -AppName as the self-hosting option it always quietly was. The registration lands in YOUR enterprise applications list, so it should carry your name for it, not a vendor's. Two traps are closed with it: the CA Documenter rename fallback no longer fires when a name was given on the command line - it is Limon-IT's own history, and searching for it in someone else's tenant could rename an unrelated app into theirs - and creating under a chosen name now prints what re-running with a DIFFERENT name would do, namely make a second registration with a new client ID and strand the consent recorded against the first." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "memberOf retirement", text: "Severity for these comes from whether the policy is actually handing something out. A policy with requestAccessForAllowedTargets set is auto-assigning an access package TODAY, so a frozen rule means new joiners silently stop receiving it - that is critical. One that is defined but not assigning is high. And removeAccessWhenTargetLeavesAllowedTargets gets called out on its own, because it is the quiet half: with the rule frozen nobody ever leaves the scope, so access that should have been revoked never is, while the setting still reads as on." },
      { kind: "improved", tool: "memberOf retirement", text: "Clean now means something stricter. The report tracks whether all three surfaces were actually read, and says so in those words: all three read and nothing found is nothing to migrate; two of three is NOT A CLEAN TENANT YET, with the unread surface named and the reason given. A surface nobody looked at is not a surface with nothing on it, and the green card is no longer available to a run that only looked at part of the problem." },
      { kind: "improved", tool: "memberOf retirement", text: "Reading entitlement management needs EntitlementManagement.Read.All, asked once on the run click while the gesture is fresh, exactly as the SMS and voice tool asks for its registration scope. Declining it is an answer rather than an error: the run carries on and that surface reports NOT READ. The read itself goes through the same candidate ladder the User or Group analyzer uses, because the relationship name moved between beta and v1.0 and ENCA runs on beta. Still reads only, still writes nothing." },
    ],
  },
  {
    build: 25225, date: "2026-08-27", title: "The rule that stops updating without telling anybody",
    items: [
      { kind: "new", tool: "memberOf retirement", text: "A second temporary tool, for a deadline that makes no noise. The memberOf dynamic rule operator has been in public preview since 2022 and Microsoft ends that preview on 3 November 2026. Rules using it do NOT fail on that date - they stop updating and stay in their last known state. A group frozen that day keeps handing out whatever membership it held: joiners are never added, leavers are never removed, and nothing anywhere on screen says anything. The tool reads every dynamic group and dynamic administrative unit in the tenant, finds the ones whose rule uses the operator, and counts down to the date." },
      { kind: "new", tool: "memberOf retirement", text: "It answers the question a scan cannot: what does it COST when this rule freezes. ENCA already holds the tenant's Conditional Access policies, so every affected group is crossed against them - and the two directions are not the same problem. A frozen group in an ENFORCED policy's EXCLUDE is a bypass that will never shrink again. A frozen group in an ENFORCED policy's INCLUDE is an enforcement gap, where the people who should be covered simply never arrive. Both are named per policy, with the policy's own state next to them. Group-based licensing is read for the same reason, and a rule that already points at a source group the directory no longer has is flagged as stale TODAY, months before the deadline." },
      { kind: "new", tool: "memberOf retirement", text: "The operator is detected by its documented user.memberof and device.memberof shape rather than the bare word, so an extension attribute somebody named memberOfDivision does not become a finding. Anything that says memberof WITHOUT matching that shape comes back as read-by-hand instead of being dropped: a scanner that quietly discards what it is unsure about is a scanner whose clean report cannot be trusted, and an empty result here is meant to mean something. Restricted-management administrative units are called out separately, because their membership decides who is shielded from tenant-wide admins." },
      { kind: "new", tool: "memberOf retirement", text: "READS ONLY, and asks for no extra consent - every read is covered by the permissions already granted. Rewriting a membership rule changes who is in a group and therefore who a Conditional Access policy hits, so the fix stays with a person in the portal. What the tool does instead is hand you the people to talk to: the owners of the affected groups, with a ready-to-send email asking each of them to replace the rule, convert the group to assigned, or say it can go. Administrative units have no owner in Entra and the modal says so rather than counting them silently. Markdown report and full CSV export, as everywhere else." },
      { kind: "new", tool: "memberOf retirement", text: "It also says what it does NOT read. Microsoft names three surfaces that can carry a memberOf rule; this tool reads two of them - dynamic groups and dynamic administrative units - and states on screen, in the report and in the export that entitlement-management auto-assignment policies are the third and are not covered here, with links to Microsoft's own script and to EMOS, which covers all three. A clean report from a tool that only looked at two thirds of the problem is worse than no report." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "SMS & voice retirement", text: "The write says what it is not. Pausing is tenant-wide and buys time to move people off SMS and voice on your own schedule; it does NOT move the 1 February 2027 retirement, after which Microsoft-provided SMS and voice stop and a phone-only user is blocked until they register a passkey. That sentence is the acknowledgement you tick before either button runs, and it changes when the tenant is already paused. Policy.ReadWrite.AuthenticationMethod is asked for on the click, Authentication Policy Administrator is named as the least-privileged role, and the result is verified by reading the property back — a tenant that does not expose it can answer the PATCH with a 204 and change nothing, which the panel now says out loud rather than reporting success." },
      { kind: "improved", tool: "SMS & voice retirement", text: "The tool is no longer described as read-only, on its tile or in its own header, because it now has exactly one write and hiding that would be worse than having it. Everything else in it still only reads. Background for the property and its three states: Roy Klooster, rksolutions.nl." },
    ],
  },
  {
