...
const CHANGELOG = [
  {
    ...
    items: [
      { kind: "fixed", tool: "Named locations", text: "<b>The untrusted-IP finding was nagging about <code>_Blocked IPs</code> — the exact case 25152 said it suppressed.</b> An underscore is a <i>word</i> character in a JavaScript regular expression, so <code>\\bblock</code> has no boundary to find in <code>_Blocked IPs</code> and the suppression never fired. A leading underscore is the usual way of pinning a list to the top of an alphabetical list in Entra, which makes it the single most likely spelling of the case the suppression exists for — so the check cried wolf about precisely the normal case it was written to stay quiet about. The boundaries are now explicit “not a letter or digit”, which an underscore satisfies, and <code>blocklist</code> as one word is matched too." },
      { kind: "fixed", tool: "Named locations", text: "Same check, opposite direction: <b><code>bad</code> is no longer a block-list word</b>. It suppressed the finding for <b>Bad Homburg office</b> — a real place, and a location whose trusted flag somebody may genuinely want to hear about. A check that silently ignores a location because of where it is would be a worse failure than the nagging it was added to prevent. The IPv6 threshold comment said <code>/48</code> while the code has always used <code>/32</code>; the code is right — a /32 is what an ISP is handed by its RIR, while a site is normally given a /48 — and the comment now says so." },
  ...
  },
  {
    ...
    items: [
      { kind: "new", tool: "All tools", text: "The roadmap items carry <b>Rnn</b> references so one can be pointed at without quoting its title. The tools went by name and emoji, which drift — tools get renamed, retitled, and once 🇳🇱 R26 lands, translated, at which point every English note about a tool stops being findable. Each tool now carries a permanent <b>T-number</b>, shown in the corner of its tile and in the tool's own header beside the per-tool version. The two sit together because they are opposite kinds of fact and are read together: the number never changes, the version always does." },
      { kind: "new", tool: "All tools", text: "<b>T01–T31 were reconstructed from git, not assigned by preference</b> — the first commit that put each tile in <code>index.html</code>, ordered by commit time, with tools that arrived in one commit ordered as they were written into the page. That makes the order a fact about the repository rather than a judgement, and it puts 🗂 List Policies, 📄 Create documentation, 🔍 Gap analyse and 🗄 Backup at T01–T04: the day this app existed at all." },
      { kind: "improved", tool: "All tools", text: "<b>Never reused</b> — not when a tool is retired, not when it is folded into another, because a recycled number makes every older note about it wrong, which is precisely the failure the number exists to prevent. A new tool takes the next free number (<b>32</b> next) in the same commit that adds its tile, alongside the changelog entry and the promotion-queue row. The rule is written where the numbers live, in <code>version.js</code>, rather than in a card somebody has to find." },
      { kind: "improved", tool: "All tools", text: "<b>Findable in the ⌨️ command palette:</b> type <code>T07</code>, or just <code>7</code>, and you land on 📘 MS Learn checks. An exact number match outranks every other kind of match, because a query that <i>is</i> a tool number is not an accident — and the bare digit works because somebody quoting a number out of a support case should not have to remember the prefix. Each tool row in the palette carries its number." },
      { kind: "improved", tool: "All tools", text: "<b>Three things are deliberately not numbered:</b> ❓ Help, 📋 What's new and 🗺 Roadmap. They are the app describing itself rather than tools that read a tenant, report on it or write to it — which is also why they carry no version. They go by name, and it is written down so nobody later closes the gap by numbering them and shifting everything else." },
      { kind: "improved", tool: "Roadmap", text: "<b>R33 moves to Now, live · beta 25154.</b> It was filed under <i>On the horizon</i>; it turned out to need no new machinery at all — a field beside each tool's version, and the discipline written down next to it." },
    ],
  },
  {
    build: 25153, date: "2026-08-19", title: "R28 — a tenant's own groups can go in the persona vaults",
    items: [
      { kind: "new", tool: "Restricted AUs", text: "<b>🏷 Group personas</b> — a per-tenant mapping from a group to the persona vault it belongs in. Everything here routed a group by reading the <b>CA number in its name</b>, which works for the baseline and nothing else: <code>SEC-VIP-Exceptions</code> and <code>Contractors-NoMFA</code> matched no persona, so ⑥ Protect skipped them, ＋ Bulk add never offered them, and break-glass had to be special-cased by name to work at all. Most tenants have groups that predate the baseline, and telling them their naming is wrong is not a feature. Say it once, in 🛡 Restricted AUs, and every tool routes it there afterwards." },
      { kind: "improved", tool: "Restricted AUs", text: "<b>One function decides</b> — <code>CaMap.codeOf</code>: this tenant's mapping first, the CA number second. ⑥ Protect, ＋ Bulk add, the persona chips, 📥 Import, the 🛡 Markdown report and the restricted-unit pages in 📄 Create documentation all go through it, so none of them can hold a different opinion about where a group belongs. The chips and ＋ Bulk add read mapped groups <b>by id</b>, one lookup each, because their scan is deliberately bounded to the CAB-SEC / CAD-SEC family and could never reach a group called <code>Contractors-NoMFA</code> however well it was mapped." },
      { kind: "improved", tool: "Restricted AUs", text: "<b>It does not guess.</b> Matching is exact: the group's object id, or its display name compared case-insensitively. Nothing is inferred from a prefix or a word in the name — a mapping nobody stated is how a group ends up in the wrong vault silently, and a vault is an authorisation boundary, so the cost of a wrong guess is one persona's scoped administrator holding another persona's exclusions. What the tenant states <b>wins over the CA number</b>, because somebody typed it against this group in this tenant, and ⑥ Protect marks such a row <i>mapped here</i> so a group filed somewhere its name does not explain always says why." },
      { kind: "improved", tool: "Restricted AUs", text: "<b>And it does not hide the unmapped.</b> 🔎 Find the unmapped groups resolves the group ids the loaded policies already reference and lists every one that nothing places, with a persona a click away — the tool reads that itself rather than borrowing the ① Check scan, so it works on a tenant where that scan was never run. A group whose id no longer resolves is deliberately left out: that is a <b>dangling reference</b> and ① Check's finding, and listing it here would send somebody to file a group that is gone. ⑥ Protect now says <i>unmapped</i> on the row instead of quietly skipping it." },
      { kind: "improved", tool: "Restricted AUs", text: "<b>Kept with the tenant, not in the code</b> — the browser's own storage keyed on the tenant <i>id</i> (not its name: two customers can share a display name, and a mapping landing in the wrong tenant would file groups into the wrong vaults). Nothing is written to your directory, nothing leaves the machine, and the site goes on storing nothing server-side. The two consequences are stated rather than hidden — it does not follow you to another browser, and a private window keeps it for the session only, which the panel says outright when storage is refused. <b>⭳ Export / ⭱ Import</b> is the way round both, with replace or merge offered rather than chosen for you; filing a group from the unmapped list records the <b>object id</b>, which survives a rename in Entra as a typed name does not. 🛡 Restricted AUs is v1.7, 📥 Import v2.7." },
      { kind: "improved", tool: "Roadmap", text: "<b>R28 moves to Now, live · beta 25153.</b> It was reverted once, at build 25100, for being built as a stored mapping in each unit's description — the wrong reading of what \"kept with the tenant\" meant. The card now records what the mapping is, the two things it refuses to do, and where it actually lives. <span>R36</span> is updated too: the per-baseline naming contract it needs now has the tenant half solved, and the same shape to copy." },
    ],
  },
  {
    build: 25152, date: "2026-08-19", title: "R37 — the named locations tool finally says what is wrong with them",
    items: [
      { kind: "new", tool: "Named locations", text: "The tool already resolved which policies use each location, counted the ones reachable only through <b>All trusted locations</b>, and warned at edit time that flipping the trusted flag moves every policy consuming it — and none of it became a <b>finding</b>. Four checks now run over exactly that data: the location list and the policies already in memory, <b>no extra read of your tenant</b>, which is why this went ahead of anything needing new Graph calls. They surface in a panel above the list, as a ⚠ badge on each row, in the per-location report and in the Markdown export — findings ahead of the inventory table, because the findings are why the file was opened." },
      { kind: "new", tool: "Named locations", text: "<b>A dangling reference</b> — a policy naming a location id this tenant no longer has, the same failure ① Check reports for groups and the same cause: something was tidied up while a policy still pointed at it. Entra keeps the stale id, so the condition reads as configured while matching nothing — for an EXCLUDE that means the exclusion silently stopped applying. High while an enforced policy still names it, medium when only disabled ones do, because that one comes back to life the day somebody switches the policy on." },
      { kind: "new", tool: "Named locations", text: "<b>An empty country location</b> matches no sign-in at all, and still reads as configured from every screen: a block scoped to it blocks nobody, an exclusion built on it excludes nobody. A location that includes <i>unknown</i> regions is deliberately not flagged — that one does match something. <b>An overly broad IP range</b>: a <code>/0</code> is the whole internet and a <code>/8</code> is 16.7 million addresses, neither of which is an office (for IPv6 the comparable line is a <code>/32</code> allocation). A <b>trusted</b> location carrying one is raised a level, because every policy using All trusted locations inherits the range without ever naming it." },
      { kind: "new", tool: "Named locations", text: "<b>The untrusted-IP check is the one that had to be careful.</b> The trusted flag only changes behaviour when something actually consumes <i>All trusted locations</i> — so in a tenant where nothing does, the check says nothing at all rather than filling the screen with a preference. And an untrusted location is very often deliberate: where a <b>Block policy names it</b>, or the name says so itself, nothing is raised, because <code>_Blocked IPs</code> is untrusted on purpose and marking it trusted would be the bug. Everywhere else it is <b>low</b>, and the finding text names the deliberate block list as a valid reason to leave it exactly as it is. A check that cries wolf about the normal case is worse than no check." },
      { kind: "improved", tool: "Named locations", text: "The findings panel is <b>not</b> part of the row filter: a dangling reference belongs to no location, so a panel that came and went with the filter would hide the one finding with no row to hide behind. A ⚠ Findings chip narrows the list to the locations that have one, the badge and the panel both open the same per-location report, and the screen and the exported report read from the same computed result so they cannot disagree. The check logic is a pure function of (locations, policies) in <code>locations.js</code> — no DOM, no globals, no reads of its own — testable with <code>node</code> alone. 🌐 Named locations is v1.6." },
      { kind: "improved", tool: "Roadmap", text: "<b>R37 moves to Now, live · beta 25152.</b> The card keeps the open question open rather than quietly deciding it: findings are deliberately NOT in 🔍 Gap analyse, because that tool reads policies and this reads objects — a first non-policy finding there needs its own answer to what a finding is <i>about</i>. There is now an implementation to argue from." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Backup (JSON)", text: "<b>Dependencies follow the scope, not the selection.</b> The policies are filtered first and the groups, authentication strengths, named locations, authentication contexts and terms of use are read from the survivors — which is the whole safeguard, because a skipped policy that still contributed its groups would put objects in the catalog that only ever existed in one tenant. That zip gets imported into a customer months later, and by then the question “why is this group here?” has no one left to ask." },
      { kind: "improved", tool: "Backup (JSON)", text: "The scope survives the download. A baseline backup is named <code>ConditionalAccess-BASELINE-…</code> rather than <code>-JSON-…</code>, and carries a <code>BackupScope.json</code> naming the tenant, the policy count and how many were deliberately left out. A zip sits in a downloads folder long after the tenant that produced it is out of reach, and “is this the whole tenant or the catalog?” is not a question a folder of policy JSON can answer. 📥 Import skips the file by name — it describes the backup, it is not content in it." },
      { kind: "improved", tool: "Backup (JSON)", text: "The scoping follows the <b>zip</b>, not the tile. 📄 Create documentation's export modal offers the same JSON backup as one of its formats — the identical file, through a different door — and it now goes through the same filter. Which tile somebody happened to open is not a decision about what ends up in a baseline." },
      { kind: "improved", tool: "Backup (JSON)", text: "There is deliberately <b>no override</b> — no checkbox to take everything just this once. The header badge and the Help section both state the behaviour, and a selection that contains no baseline policy at all is refused with the reason instead of quietly producing an empty zip. 🗄 Backup is v1.3." },
    ],
  },
  {
    build: 25149, date: "2026-08-19", title: "Import stops walking past the groups it reuses (R04)",
    items: [
      { kind: "new", tool: "Import", text: "A group this import CREATES gets nesting disabled and a place in its persona vault. A group it REUSES got neither \u2014 and build 25129 only said so, which turned an invisible gap into a visible one without closing it: the panel told you the group was exposed and then sent you to two other tools. Each reusable row now carries a <b>tick</b> for what is genuinely still open on it \u2014 \ud83d\udeab disable nesting, \ud83d\udd12 file it into its persona vault \u2014 and ticking is the whole interface: <b>nothing is pre-ticked</b>. An existing group may hold members and sit somewhere deliberately, so the decision stays with the person who knows the tenant, not with the importer." },
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "new", tool: "Create documentation", text: "The questions an auditor arrives with lead the output: <b>who could widen an exclusion</b> and <b>what would happen if they did</b>. A unit with nobody scoped, an empty unit and a role-assignable group frozen inside a restricted unit are findings rather than footnotes. Failed reads remain unknown — never silently re-labelled empty or safe — and policy impact is calculated from <b>all</b> tenant policies even when the exported policy-page selection is narrower." },
      { kind: "improved", tool: "Create documentation", text: "The restricted-unit integration is optional and deliberately fail-soft. Create documentation reads the unit list, members, scoped roles and role definitions for itself; it does not depend on the Restricted AUs screen having run or on state another tool left behind. A missing permission, unavailable endpoint, unreadable unit or supplemental-page render failure is recorded or skipped locally while the core policy document continues to export. The existing Restricted AUs <b>Export MD</b> stays the raw inventory it has always been." },
      { kind: "fixed", tool: "Roadmap", text: "R14 — the single-tenant app registration — said <i>available now</i> while still sitting inside Next. It shipped in beta build 25014 and reached production in 273, so it now lives in the shipped timeline where the roadmap's own rule says it belongs. R27 moves from Next to Now with this build." },
      ...
      { kind: "fixed", tool: "Device reality check", text: "Help ended by saying coverage was judged only from assignment targets and that memberships were not expanded, directly contradicting the membership pass added in build 25110. It now describes the actual ladder: direct assignments first, transitive user-overlap matching when names cannot prove coverage, and <i>not proven</i> when a scope cannot be read safely. The Permissions overview now names Device reality check beside both Intune read scopes it asks for." },
      { kind: "improved", tool: "Change audit", text: "The home tile and both rendered headers now carry <b>UPDATED</b> for the new short incident windows. Updated and new tools are always kept in view on the collapsed home page; an untagged change is an invisible change." },
    ],
  },
  {
