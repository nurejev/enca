...
const CHANGELOG = [
  {
    ...
    items: [
      { kind: "new", tool: "Licence gap", text: "Promotion item 61 — a new tool, in <b>BETA</b>. Microsoft has begun showing a <i>Licensing overage</i> warning on the Conditional Access page, linked to the licence usage blade — and the blade counts the users who <b>triggered</b> a policy last month, while the obligation is on the users your policies <b>target</b>. 🎫 Licence gap computes the second number: every active policy's targeting resolved on real members (All-users minus resolvable exclusions, or the union of included users, transitive group members and directory-role holders), the cross-policy union compared with the P1/P2 seats you actually own, matched on the <code>AAD_PREMIUM</code> service plans so bundles count and suspended subscriptions do not. Risk-based policies are named as the P2 obligation, and a disabled one is named as a P2 obligation waiting to happen." },
      { kind: "new", tool: "Licence gap", text: "<b>The gap is named, not just counted.</b> Every targeted user without the licence assigned is listed by UPN, on screen in a searchable pop-out and in the Markdown export, sorted into what you would actually do about each: buy a seat, clean up a disabled account, or leave a shared / room / equipment mailbox alone. 🏷 Check mailbox types asks for <code>MailboxSettings.Read</code> on the click and labels those resource accounts — and where the delegated read is refused, the refusal itself is the answer, because a regular unlicensed user has no mailbox to be refused access to. Admin-accounts groups can be excluded, several at once, because Microsoft licenses <i>people</i> and a second internal account of an already-licensed person needs no second seat." },
      { kind: "improved", tool: "Licence gap", text: "<b>Honest numbers throughout.</b> A count where a group was unreadable or over the read cap, or a role could not be resolved, is marked <b>≈</b> rather than presented as exact; guests are deliberately not counted; and a user whose only P1/P2 comes from a plan still in grace on a cancelled subscription is IN the gap and labelled as such, because that seat is no longer owned. Reads only, no extra consent beyond the mailbox check. After Rudy Mens' <code>Get-EntraLicenseGap.ps1</code> (lazyadmin.nl)." },
      { kind: "new", tool: "Sign-in failures", text: "Promotion item 62 — enforced mode now also gathers the sign-ins Conditional Access <b>interrupted</b>: an abandoned MFA prompt, an MFA enrolment, a device authentication, a terms-of-use acceptance nobody completed. Graph has no “interrupted” Conditional Access status — those records carry <b>success</b> plus an interrupt error code — so a second server-filtered read on the interrupt codes is deduped into the failures. An interrupt with no Conditional Access control behind it (per-user MFA, for instance) is dropped rather than blamed on a policy." },
      ...
      { kind: "new", tool: "Import", text: "Promotion item 63 (R04) — a group this import <b>creates</b> gets nesting disabled and a place in its persona vault; a group it <b>reuses</b> got neither, and merely naming that turned an invisible gap into a visible one without closing it. Each reusable row now carries a tick for what is genuinely still open on it — 🚫 disable nesting, 🔒 file it into its persona vault — and <b>nothing is pre-ticked</b>: an existing group may hold members and sit somewhere deliberately, so the decision stays with the person who knows the tenant. What cannot be done says why on the row instead of offering a dead checkbox." },
      { kind: "improved", tool: "Import", text: "The writes run <b>after</b> the policies land, so an import that fails leaves every existing group exactly as it found it, and membership is never touched. Neither write is destructive — Entra refuses the nesting change when a group already holds nested groups rather than forcing it, and administrative-unit membership can be undone — while the one route that is destructive stays in ⑧ Disable nesting behind its own typed confirmation. Nothing is reported as done unless it was read back and confirmed. <b>Roadmap R04 is live.</b>" },
      { kind: "new", tool: "Backup (JSON)", text: "Promotion item 64 — a baseline tenant is where the baseline is <b>built</b>, so a backup taken there is the <b>baseline catalog</b>, and it now behaves like one: scoped to the persona CAxxx policies, the same rule the 🔍 Gap and 📚 MS Learn checks already apply on those tenants. The policies are filtered <i>before</i> the dependencies are gathered, which is the whole safeguard — a skipped policy that still contributed its groups would put objects in the catalog that only ever existed in one tenant, and that zip gets imported into a customer months later. The zip is named <code>ConditionalAccess-BASELINE-…</code> and carries a <code>BackupScope.json</code> saying what was left out. No override, by design. Inert on every customer tenant." },
      { kind: "improved", tool: "Create documentation", text: "📄 Create documentation goes through the same helper, because these two had to ship together: scoping the zip and not the Word file leaves the tenant half-covered, and the Word file is the one that gets forwarded. The export modal states the scope and the skipped count, and every format carries it into the deliverable itself — PDF cover line, Markdown blockquote, Word title, a <code>SCOPE.txt</code> beside the PNGs — from one shared sentence. The restricted-unit appendix is deliberately <b>not</b> filtered: on a baseline tenant those units are the baseline's own persona vaults." },
      { kind: "improved", tool: "Baseline Policies", text: "Promotion item 65 (R35) — the baseline bundled here is published as the <b>CloudFellows baseline — R26.6 (v3.x)</b>. It was the <i>Limon-IT baseline</i>; the release designation, the 99 policies and every version in them are unchanged. The catalog's <code>label</code> and <code>author</code> are what the tool header, the summary line and the Markdown gap report each read, so none of the three could be left behind. Two things deliberately untouched: the catalog <b>id</b>, which saved state and 📉 Drift watch snapshots key on — renaming it would have orphaned every stored comparison — and the <b>CAB-SEC / CAD-SEC group names</b>, which are the tenant's own objects and not ours to rename. Display name everywhere, identifiers nowhere." },
      { kind: "new", tool: "Named locations", text: "Promotion item 66 (R37) — the tool already resolved which policies use each location, counted the ones reachable only through <b>All trusted locations</b>, and warned at edit time that flipping the trusted flag moves every policy consuming it. None of it became a <b>finding</b>. Four checks now run over exactly that data — <b>no extra read of your tenant</b>: a <b>dangling reference</b> (a policy naming a location id this tenant no longer has, so an exclusion built on it silently stopped applying while the policy still reads as configured), an <b>empty country location</b> (matches nobody while every policy using it looks scoped), and an <b>overly broad IP range</b> (a <code>/0</code> is the whole internet and a <code>/8</code> is 16.7 million addresses; raised a level when the location is trusted, because All-trusted policies inherit it unnamed)." },
      { kind: "improved", tool: "Named locations", text: "The fourth check is the one that had to be careful. An <b>untrusted IP location</b> is only worth mentioning where policies actually rely on “All trusted locations”, so in a tenant where nothing does it says nothing at all. And an untrusted location is very often deliberate: where a Block policy names it, or the name says so itself, nothing is raised — <code>_Blocked IPs</code> is untrusted on purpose and marking it trusted would be the bug. Everywhere else it is raised as low, and the text names the deliberate block list as a valid reason to leave it exactly as it is. A check that cries wolf about the normal case is worse than no check. Findings appear in a panel above the list, as a ⚠ badge per row, in the per-location report and ahead of the inventory table in the Markdown export — deliberately <b>not</b> in 🔍 Gap analyse, which reads policies while this reads objects." },
      { kind: "new", tool: "All tools", text: "Promotion item 68 (R33) — every tool now carries a permanent <b>T-number</b>, in the corner of its tile and in the tool's own header beside the per-tool version. The two sit together because they are opposite kinds of fact: the number never changes, the version always does. T01–T31 were reconstructed from the repository rather than assigned by preference — the first commit that added each tile, in commit order — which puts 🗂 List Policies, 📄 Create documentation, 🔍 Gap analyse and 🗄 Backup at T01–T04. Findable in the ⌨️ command palette: type <code>T07</code>, or just <code>7</code>, and you land on 📘 MS Learn checks." },
      { kind: "improved", tool: "All tools", text: "<b>Never reused</b> — not when a tool is retired, not when it is folded into another, because a recycled number makes every older note about it wrong, which is the exact failure the number exists to prevent. Production has <b>two gaps in the sequence, and they are correct</b>: T21 and T29 belong to 📐 CIS Benchmark and 📖 Baseline guide, which are still proving themselves on the beta channel. The numbers are theirs from the day they were assigned, whether or not this site carries the tool; when those two graduate they arrive as T21 and T29. ❓ Help, 📋 What's new and 🗺 Roadmap carry no number at all — they are the app describing itself rather than tools that read a tenant, which is also why they carry no version." },
      { kind: "improved", tool: "Baseline Policies", text: "Promotion item 69 — the Cards / Table toggle is gone and the <b>table is the only view</b>, in both catalogs. Cards were the older idea and the table had quietly become the one people used: on a 99-policy catalog the card grid is a scroll, not a comparison. The status filters, the search, Export MD, Refresh and Import baseline are untouched. In the same change, <b>Collapse all</b> works under the <i>Not in baseline</i> filter — it was selecting the card view's row set, which holds baseline entries only, so with that chip active the button did nothing at all. 🧬 Baseline Policies is v2.0 and 🧩 Baseline (Joey Verlinden) v1.2." },
      ...
      { kind: "fixed", tool: "Conditional Access groups", text: "<b>Three things this channel had been told it already had.</b> 📘 MS Learn's ＋ Create group still passed <code>roleAssignable: true</code> on its fallback path — so production had a live route that built exactly the object R04 exists to retire, on the build where R04 is declared live, while the Help and the version notes both said it could no longer happen. It creates a plain security group now, as every other create path here already did. Ported from beta 25026, where it has been running for over a hundred builds; the promotion that carried the rest of that change missed this one call." },
      { kind: "fixed", tool: "Conditional Access groups", text: "⑦ Migrate's member copy read <code>/members/microsoft.graph.user</code> — <b>users only</b> — while this tool's own version note has claimed since build 219 that it moves every member type and reports the nested groups it cannot. A persona group holding service principals or devices lost them silently during a recreate, and the report said the migration was clean. It now reads all members, moves service principals and devices with the users, and lists any nested group Entra forbids it to move. Ported from beta 22904, missed by the same promotion." },
      { kind: "fixed", tool: "Named locations", text: "The country picker's chips had no styling at all here: <code>.lo-chips</code> and <code>.lo-chip</code> were never carried across with R08 in build 269, so the codes you had picked rendered as a run of bare text with stray ✕ characters. The rules are in. Also in this pass: the 📉 Drift watch help finally says that snapshots include the <b>administrative units</b> — <code>drift.js</code> has read them since the tool reached 1.0 and the help listed four areas instead of five — the 👥 CA groups help documents the ＋ Add bar on the members matrix, the consent table names 🛡 Restricted AUs alongside CA groups as a consumer of <code>AdministrativeUnit.ReadWrite.All</code>, ⑤ Import members clears its log so a stale one cannot survive into the next run, and the stylesheet is reconciled with beta rule for rule — a block of What-If styles that existed twice is down to one copy, with nothing gained or lost besides the baseline-card rules that item 69 deliberately retired." },
      { kind: "improved", tool: "Roadmap", text: "<b>R04, R33, R35 and R37 move to Now, live · build 287.</b> R28 — custom groups in the persona vaults — is deliberately <b>not</b> in this release: it is built and running on the beta channel, and it is the one change in this batch that alters where a <i>write</i> puts a group object, so it graduates only once a real tenant has mapped a group and the placement has been checked in the unit afterwards. 📐 CIS Benchmark and 📖 Baseline guide remain beta-only." },
    ],
  },
  {
