...
const PROMOTE = {
  ...
  items: [
    {
      n: 95,
      title: "PVM demo retired from the repository",
      tools: ["All tools"],
      builds: [25196],
      risk: "medium",
      what: "The Perfetti Van Melle per-audience entry leaves: the BRAND_OVERRIDES object in js/branding.js, the /pvm front-door page and assets/pvm are all removed from the repo and gitignored (the folders stay on the local machine, offline only). The whole look was exported first into pvm/selfhost-branding.json - logos as embedded data URIs, both full palettes - so PVM can wear it on a self-hosted instance by serving that one file. BRAND_OVERRIDES ships empty; forUpn, ?brand= and the stylesheet injection machinery stay.",
      why: "It was a demo of the override mechanism and the demo is done. The risk is not code - an empty list is the code path every non-PVM session already took - it is REACH: promoting this deletes the live /pvm page and the UPN-triggered look from the production site, so anyone still using that entry point loses it. Graduates when Mihai confirms PVM is told and self-hosting (or nothing) is their path. Promotion note: main carries its own copies - the port must git rm pvm/ and assets/pvm/ there, remove the branding.js entry, and carry the .gitignore lines, or the next contributor re-adds the folder by accident.",
      test: [
        "Beta site after deploy: /pvm/ returns 404, and no file under assets/pvm is reachable.",
        "View the served js/branding.js: BRAND_OVERRIDES is empty - no Perfetti string, no UPN domain, no palette.",
        "Sign in with any account (a pvmict.com one if available - say so if skipped): default ENCA look, no errors from forUpn on the empty list.",
        "?brand=pvm in the URL: ignored cleanly - default look, no console error, no stale sessionStorage key kept.",
        "Serve pvm/selfhost-branding.json as /selfhost-branding.json on a local instance: the full PVM look renders in light AND dark, wordmark and favicon included - proving the export is complete before the repo copy is gone from main too.",
    ...
    },
    {
      ...
      test: [
        ...
        "Exports (any Markdown/PDF) still carry the neutral ENCA credit under the custom branding - the override is chrome only, same as PVM.",
        ...
        "Import a full-palette branding file (the retired PVM export is one): the form prefills, NOTHING changes until Apply, and after Apply + Download the round-tripped file still carries every palette entry, not just the five picker colours.",
