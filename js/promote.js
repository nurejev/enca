  ...
  items: [
    {
      ...
      builds: [25225, 25226],
      risk: "low",
      what: "New temporary tool for the 3 November 2026 retirement of the memberOf dynamic rule operator. Reads all THREE surfaces Microsoft names: the tenant's dynamic groups (server-side filtered to DynamicMembership), its dynamic administrative units, and its entitlement-management auto-assignment policies (25226 - through the same candidate ladder js/groupuse.js uses, keeping only policies that carry automaticRequestSettings, the way Microsoft's own script does). Detects the operator by its documented user.memberof / device.memberof shape and reports a memberof that does NOT match that shape as read-by-hand rather than dropping it. Crosses every affected group against the Conditional Access policies ENCA already holds - an EXCLUSION that stops shrinking is a permanent bypass, an INCLUSION that stops growing is an enforcement gap - and adds group-based licensing, membershipRuleProcessingState, and the source groups each rule names, so a rule pointing at a deleted source group is flagged as stale today. For an entitlement-management policy the severity comes from requestAccessForAllowedTargets, with removeAccessWhenTargetLeavesAllowedTargets called out separately. Verdicts critical / high / review / read-by-hand, an allSurfaces fact that decides whether a clean result may say nothing to migrate or must say not a clean tenant yet, owner notification email, Markdown report, CSV export. New file js/memberof.js, screen and tile in index.html, wiring and TOOL_TABS entry in js/app.js, TOOL_VERSIONS toolMemberOf t:34.",
      why: "READS ONLY and nothing in the tool writes, so the risk of shipping it is a WRONG REPORT rather than a changed tenant. That is still worth holding for a cycle, and the reason is sharper now that all three surfaces are in: the operator-detection regex, the enforced/report-only split and the auto-assigning split decide whether a row says critical, and a false CLEAN here is the one output nobody would question. The 25226 entitlement-management read is also the only part that asks for a permission (EntitlementManagement.Read.All, on the run click) and the only part whose Graph shape differs between beta and v1.0, so it wants a real tenant under it before a customer sees it. Graduates once it has been run against a tenant that genuinely uses memberOf on more than one surface and the findings have been checked by hand against the Entra portal.",
      test: [
        ...
        "DECLINE the EntitlementManagement.Read.All prompt on the run click. The run must carry on to completion, the notes must say entitlement management NOT read with the reason, and the caveat line must say the result covers 2 of the 3 surfaces. Nothing may report zero auto-assignment policies.",
        "Then GRANT it on a tenant with entitlement management in use, and confirm the count of auto-assignment policies read is non-zero and matches what the Entra admin center shows under Identity Governance. If the count is zero on a tenant that has them, the candidate ladder picked the wrong relationship name for this cloud - check the browser console for the failing URLs.",
        "On a tenant with an auto-assignment policy whose rule uses memberOf: confirm the row names the ACCESS PACKAGE, not only the policy, and that a policy with requestAccessForAllowedTargets on comes back critical while one with it off comes back high. Toggle that setting in the portal and rescan - the verdict must move.",
        "Confirm a REQUEST-BASED assignment policy never appears. Only policies carrying automaticRequestSettings have a rule that can freeze; a request-based policy in the table would be a false finding, and this is the filter Microsoft's own script applies too.",
        "The clean-tenant wording is now conditional and both halves need seeing. With all three surfaces read and nothing found, the card must be the green nothing-to-migrate one and must say all three were read. With any surface unread and nothing found, it must instead say NOT A CLEAN TENANT YET and name the unread surface. A green card on a partial scan is the failure this check exists for.",
        ...
        "Export both. The CSV must carry one row per affected object with the full membership rule intact through the quoting, an accessPackage column filled for entitlement-management rows and empty for the others, and autoAssigning / removesAccessOnLeave filled only for those rows. The Markdown must state which surfaces were read: the all-three sentence when they all were, and a What was not read section naming each gap when they were not. A report that claims more coverage than the run actually had is the one bug here nobody would catch downstream.",
      ],
