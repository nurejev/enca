...
const PROMOTE = {
  ...
  items: [
    ...
    {
      n: 117,
      title: "🌿 A greener page background in light mode",
      tools: ["All tools"],
      builds: [25238],
      risk: "low",
      what: "One token in css/app.css: the light theme's --bg goes from #f2f7f3, a near-white, to #dbeade, a clear mint. --soft, --surface, --border and every text colour are untouched, and neither dark-mode block is changed.",
      why: "Cosmetic and one line, but it touches EVERY page in the app, which is the only reason it is a queue item at all. --bg is not only the body background: sticky tool navs, the sticky group-analyzer header and the export stage all paint with it, so the risk is not the colour but the places that assumed the colour was almost white. Nothing here is worth blocking a release for; it is worth one honest look at the app before it ships.",
      test: [
        "Load the home page in light mode and confirm the tool tiles read as white cards ON a green page rather than a white page with faint outlines. This is the whole point of the change - if it does not look different, it did not apply, so check the ?v= too.",
        "Scroll a long tool - List Policies with a large tenant - and watch the sticky nav as rows pass under it. It paints with --bg, so if it were transparent or mismatched you would now see rows ghosting through it, where before white on near-white hid it.",
        "Do the same on the Group Analyzer's sticky header, which uses its own rule with the same variable.",
        "Export a PNG or a PNG bundle from Create documentation. The export stage paints with --bg, so exported images now carry the mint ground. Confirm that is acceptable in a document - it is the one place the change leaves the screen.",
        "Switch to dark mode and confirm NOTHING moved. If dark looks different, the wrong block was edited.",
        "Check contrast on anything that sits directly on the page rather than in a card: section headings, the muted section descriptions and any bare chips. --muted is #5f7a67 on the new ground - readable, but look rather than assume.",
      ],
      files: ["css/app.css"],
    },
    {
