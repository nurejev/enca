  ...
  items: [
    {
      ...
      test: [
        ...
        "Courseware, where both copies were made on 17 Sep: each row reads created with the time and one of them is marked newer, the other older; no set offers a tick reading Users included: None.",
        "A tenant where two copies carry the same createdDateTime, or one has none: neither row is marked, and nothing claims which came first.",
    ...
    },
    {
      ...
      test: [
        ...
        "Courseware: the six E-Admins from the 16:52 run are there, Off (a re-run lists them as skip). A fresh E-Admins import elsewhere: no row reports does-not-exist; if Graph lags, the rows take a few seconds longer and still end ✓.",
        "An E-Admins import creates no authentication context unless a chosen policy names one; the report's Created list has no Strong Authentication for PIM line. Emergency_Access1/2 show under the break-glass unit in the Protection section (or as not placed because that unit is missing), never under ADM, and no CAD-SEC-U-DG-ADM line appears.",
