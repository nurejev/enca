...
const PROMOTE = {
  ...
  items: [
    ...
    },
    {
      ...
      files: ["js/capabilities.js", "js/app.js", "tools/capabilities-write.test.cjs", "js/version.js", "js/changelog.js", "js/promote.js"],
    },
    {
      n: 274,
      ...
      test: [
        "25489: a guest MFA policy that reaches B2B direct connect users, with direct connect enabled inbound and MFA trust off, shows MFA for B2B direct connect users without inbound MFA trust; turn trust on or block direct connect inbound and it goes.",
        "25489: in 📘, every blocked cell of the shared-device matrix has a Shared devices: controls their resource accounts cannot meet finding naming the same policy and control; an All-users policy offers the Fix (exclude the group), the devices' own policy says to remove the control and offers no Fix.",
    ...
    },
    {
      ...
      tools: ["Policies"],
      builds: [25484, 25486, 25487, 25488],
      risk: "high",
      ...
      test: [
        "25488: select several Off policies → Delete → backup + DELETE: the dialog swaps the list for a ledger, each row turns ✓ with in the recycle bin for 30 days, Stop halts before the next one (rows read stopped — not deleted), Cancel reads Close, and Report opens in front with every deleted policy's id.",
        "25487: a pair where both versions are On appears under ⇄ Switch over; switching it touches only the older version (Off), even with Report-only first ticked. A newer version in Report-only beside an older On is not offered.",
        "25487: Perfetti after the 11 switch-overs: rows that differ only in assignments read Needs review with a Reviewed tick; ✓ Mark all reviewed ticks them; ☑ Select all to delete selects every reviewed or cleanup row; Review & delete opens the normal backup + typed DELETE with exactly those.",
    ...
    },
    {
      ...
      test: [
        "25490: 🛡 Checks → Microsoft Learn opens on a ▶ Run checks prompt listing the reads, with no Graph traffic; Run fills findings and both matrices; switching to Bypass and back keeps them; ⟳ Refresh re-reads and re-runs; ticking Include Off before the first run changes the note only.",
