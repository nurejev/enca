...
const PROMOTE = {
  productionBuild: "v1.0.296",

  items: [
    {
      n: 95,
      title: "Per-audience demo retired from the repository",
      tools: ["All tools"],
      builds: [25196, 25200],
      risk: "medium",
      what: "The demo audience's per-audience entry leaves: the BRAND_OVERRIDES object in js/branding.js, the /pvm front-door page and assets/pvm are all removed from the repo and gitignored (the folders stay on the local machine, offline only). The whole look was exported first into a local selfhost-branding.json - logos as embedded data URIs, both full palettes - so the customer can wear it on a self-hosted instance by serving that one file. BRAND_OVERRIDES ships empty; forUpn, ?brand= and the stylesheet injection machinery stay. Build 25200 finished the job: the customer's name and UPN domains were scrubbed from every tracked file - the old changelog entry, this queue item, a branding.js comment.",
      why: "It was a demo of the override mechanism and the demo is done. The risk is not code - an empty list is the code path every other session already took - it is REACH: promoting this deletes the live /pvm page and the UPN-triggered look from the production site, so anyone still using that entry point loses it. Graduates when Mihai confirms the customer is told and self-hosting (or nothing) is their path. Promotion note: main carries its own copies - the port must git rm pvm/ and assets/pvm/ there, remove the branding.js entry, carry the .gitignore lines, and carry the 25200 scrubs, or the next contributor re-adds the folder by accident and main keeps serving the identifiers.",
      test: [
        "Beta site after deploy: /pvm/ returns 404, and no file under assets/pvm is reachable.",
        "View the served js/branding.js: BRAND_OVERRIDES is empty - no customer name, no UPN domain, no palette.",
        "Search every served js file for the demo tenant's company name and both UPN domains: zero hits - the 25200 scrub is what this checks.",
        "Sign in with any account (one from the demo tenant if available - say so if skipped): default ENCA look, no errors from forUpn on the empty list.",
        "?brand=pvm in the URL: ignored cleanly - default look, no console error, no stale sessionStorage key kept.",
        "Serve the local pvm/selfhost-branding.json as /selfhost-branding.json on a local instance: the full demo look renders in light AND dark, wordmark and favicon included - proving the export is complete before the repo copy is gone from main too.",
        "git ls-files | grep pvm on the branch is empty, while the local working tree still has both folders (gitignored).",
      ],
      files: ["js/branding.js", "js/app.js", "js/promote.js", ".gitignore"],
    },
    {
      n: 94,
      title: "Self-host branding gear (S02)",
      tools: ["All tools"],
      builds: [25195, 25196, 25197, 25198, 25199],
      risk: "medium",
      what: "On any non-production host, a gear appears next to Sign out: product and organisation names, logos, login text and light/dark identity colours, saved to this browser or downloaded as selfhost-branding.json for the whole deployment. New js/selfhost.js, plus three small app.js hooks: the selfhost override as the fallback brand key, an id on the BETA ribbon, and an enca:brand-updated listener that repaints the chrome. 25196 fixed the gear itself: at the button's default text size it was nearly invisible, and it now draws at 21px in the same button. 25197 added Import from JSON - the dialog reads a selfhost-branding.json back into the form for review before Apply, through the same sanitiser as every other source - and made full per-theme palettes survive the dialog instead of being cut down to the five picker colours on Apply. 25198 un-hard-coded the dark chrome: the policy-card header, matrix header, selection bar, toast and page glow drew the default muted greens in dark mode whatever the brand said - they now read brand-overridable variables (pchead-a, pchead-b, panel-deep, hero-glow) with the greens as fallbacks. 25199 killed the flash-of-default on load: a blocking head script (js/selfhost-boot.js) paints the browser's known brand - the gear's saved look or a cache of the last fetched selfhost-branding.json - before first paint; the cache is stored on fetch success, cleared when the file is gone, untouched on network error, and applyBranding removes the boot stylesheet when it takes over.",
      why: "It reuses the per-audience override mechanism exactly - chrome only, exports keep the neutral credit - and it cannot appear in production at all, because the gear and the override only exist on a non-canonical host. The risk worth respecting is injection: every value ends up in the header, the login screen or a generated stylesheet, so colours are charset-allow-listed, URLs must be https, and images must be data URIs or relative paths. It graduates once the gear has dressed a real deployment without the palette or the ribbon misbehaving in either theme.",
      test: [
        "On the beta site (a non-production host): a gear renders next to Sign out after sign-in, at 21px - obviously a control, not a speck. On production there must be NO gear - view source is not enough, check the DOM.",
        "Set a name, org and both colour sets, Apply: header, chip, login screen and palette change without a reload, in light AND dark theme, and survive a reload (localStorage).",
        "Remove local branding: everything returns to the deployment's own look without a reload.",
        "Download selfhost-branding.json, serve it next to index.html (the compose file mounts it), open in a browser with clean storage: every visitor gets the branding, and the red BETA ribbon reads SELF-HOSTED on a slate background with the [SELF-HOSTED] title tag.",
        "Exports (any Markdown/PDF) still carry the neutral ENCA credit under the custom branding - the override is chrome only, same as any per-audience look.",
        "Paste a hostile value - a colour of red}body{display:none, an orgUrl of javascript:alert(1) - Apply, reload: the value is dropped, not rendered.",
        "Import a full-palette branding file (either retired-demo export on the local machine is one): the form prefills, NOTHING changes until Apply, and after Apply + Download the round-tripped file still carries every palette entry, not just the five picker colours.",
        "Import a non-JSON file and a JSON that is not a branding file: a clear refusal both times, and the dialog keeps its previous values.",
        "Branded DARK mode, with a colorsDark carrying the pchead/panel-deep/hero-glow variables: open List Policies - the card header gradient wears the brand, not green - then the Matrix view header, a selection bar and a toast. Repeat in the auto theme (OS dark, no explicit choice): identical.",
        "UNBRANDED dark mode after this change: card header, matrix header, selbar, toast and page glow are pixel-identical to before - the greens moved into fallbacks, they did not change.",
        "Flash check, branded instance: hard-refresh (cmd-shift-R) twice - the SECOND load must paint the brand's colours and logo from the first frame, no Limon-IT flash (throttle the network in devtools to make a flash unmissable). The FIRST-ever load in a fresh profile may flash once - that is the documented floor.",
        "Remove selfhost-branding.json from the server, load twice: the first load may still boot branded (stale cache), the second must not - the fetch clears the cache. An unbranded deployment never runs the boot script's paint at all (no stored brand).",
        "With branding applied then removed via the gear while the page is open: the logo returns to the default - the boot stylesheet's content:url rule must not survive applyBranding.",
        "An explicit ?brand= override key still beats the selfhost fallback (BRAND_OVERRIDES ships empty since 25196 - add a throwaway entry locally to check).",
      ],
      files: ["js/selfhost.js", "js/selfhost-boot.js", "js/app.js", "index.html", "css/app.css"],
    },
    {
      n: 93,
    ...
    },
    {
      ...
      test: [
        ...
        "A branded session (any BRAND_OVERRIDES entry, or a selfhost-branding.json): its own name and chip render, still no host line.",
