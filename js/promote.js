  ...
  items: [
    {
      ...
      test: [
        ...
        "On a tenant holding BOTH baselines (95 of 99 CloudFellows, 28 of Joey's): the CloudFellows card must read ★ Active baseline — by match, nothing saved yet, with 89 under CloudFellows names and 28 under Joey Verlinden names; its counts must show no version-unknown rows. Joey's card must read 28 present, the clashes named as CloudFellows by its naming, the not-in-baseline rows as N CloudFellows, and hovering ⚠ number clash must show the meaning. Click 📌 Keep on CloudFellows: the wording must change to saved for this tenant.",
      ],
    ...
    },
    {
      ...
      test: [
        ...
        "Give a test user with E5 a Teams Shared Space licence on the same account and rescan: the preview count must NOT include that user, the people-with-a-device-licence line must count and name them, and after the write the user must NOT appear in the group. Remove the E5 from that account and, once assignedPlans no longer lists SHAREPOINTENTERPRISE as Enabled, the account must join the group.",
        "Open the rule on a group that still has the old three-plan rule: the expanded row must say Does not keep out SHAREPOINTENTERPRISE (or the tenant's own markers), and the verdict must be update the rule even when every device plan is present.",
        "Click Write: a CA-TeamsDevices-rollback-<group>-<tenant>-<stamp>.json must download BEFORE the success line appears, containing before.membershipRule equal to the rule the dialog showed as current and a restore.body you can PATCH by hand. Click ⎌ Roll back now: the group in Entra must carry the old rule again and a rescan must show update the rule once more.",
        "On a tenant whose suites carry SHAREPOINTENTERPRISE: the Must hold none of list must name the SharePoint plans, not a plan that merely happens to be in every suite.",
