...
const PROMOTE = {
  ...
  items: [
    ...
    },
    {
      ...
      builds: [25338],
      risk: "low",
      what: "index.html: the tiles toolDocument, toolJson, toolState and toolBaselineJoey are removed; toolPolicies is renamed Policies with a blurb naming the five action-bar verbs and a writes-to-tenant tag; toolBaseline is renamed Baseline and its blurb names both catalogs behind the picker. Help: the four folded sections become h5 sub-sections under their hosts (Create documentation, Backup and Set Policy state under Policies; Baseline (Joey Verlinden) under Baseline), each keeping its emoji and name, with a help-fold note; the Tool numbers section gains the folded mapping; T01 to T31 in that section corrected to T39. css/app.css: .help-sec h5, .help-fold, .help-toc a.sub. js/app.js: a FOLDED map next to toolNoOf ({into, label, where, build} per folded id) — the md-tool click handler resolves an old tile id through it, and cpBuild offers each folded tool under its own label with the host and the place inside it in the hint, scored one below a live tool so a real tile always wins a tie; the four tile click handlers removed; the four TOOL_TABS rows removed and the two hosts relabelled; toolPolicies opens with setToolMode of empty string instead of document; toolBaseline opens on Baseline.activeCatalogId(); the data-open-baseline crumb is Baseline either way; buildHelpToc also selects h5 and marks those links sub. js/guide.js: the two link labels follow the renames. js/version.js: build 25338, T01 2.11, T10 2.6, and a folded {into, build} field plus a note line on T02 1.7.1, T04 1.3.1, T05 1.0.1, T11 2.0.2. index.html roadmap R52 shipped 25338.",
      why: "Low: no engine, no Graph read and no write changed. openExport, runBackup, openStateModal and openBaseline are untouched and keep every caller; the action bar already offered all three verbs and is visible whenever policies are loaded, so the reads still fall back to every visible policy and the writes still demand a pick. The tile version stamper and the command palette already skip a TOOL_VERSIONS entry with no tile, so a folded entry needs no guard. Two things to satisfy yourself about before it graduates: that no in-app link still points at a removed tile without a FOLDED_TO row, and that the home page reads as MORE capable rather than less — a fold that leaves the page saying less than it did has hidden something.",
      test: [
        "Home page: 35 tiles, and the four gone are 📄 Create documentation, 🗄 Backup (JSON), 🎚 Set Policy state and 🧩 Baseline (Joey Verlinden). The side nav and the + menu list the same 35 plus the three app pages; neither has a dead row.",
        "🗂 Policies: the tile reads Policies with T01 v2.11 in the corner, the blurb names Create documentation, Backup, Gap analyse, Assign groups or roles and Set policy state, and it carries the writes-to-tenant tag. Open it with NOTHING selected: the action bar is there, no button is highlighted, 📄 Documentation and 🗄 Backup act on every visible policy, and 👥 Assign and 🎚 Policy state say to select something first.",
        "Select two policies in 🗂 Policies and run each of the five bar actions in turn: the documentation modal, the backup zip, the gap matrix, the assign panel and the state dialog with its run ledger all behave as they did at 25337. Then the per-card quick actions on one policy: Documentation, Backup, Assign, Policy state.",
        "Baseline tenant (the 🧪 badge): 📄 Documentation and 🗄 Backup still scope to the persona CAxxx policies, the modal still says how many of the tenant's own are left out, and the zip is still named ConditionalAccess-BASELINE with its BackupScope.json.",
        "🧬 Baseline: one tile, T10 v2.6. It opens on the tenant's ★ active catalog — on a tenant where Joey Verlinden is active it must open on HIS table, not CloudFellows. The picker in the toolbar switches both ways, his live repository read still fires on the switch (the card names the release and commit), and the ★ switch, 📌 Keep and 🧹 what the other baseline left behind are all still on the summary card.",
        "Deep links, which is where a fold breaks quietly: 📖 Baseline guide steps 1, 5 and 6 (their links read 🧬 Baseline and 🗂 Policies now), the change-baseline links that carry data-open-baseline, and 🗣 User impact brief's stale-baseline callout. Each must land on the tool, not toast That tool is not available here.",
        "In the browser console, FOLDED_TO coverage: run [...document.querySelectorAll('[data-tool]')].map(a=>a.dataset.tool).filter(t=>!document.getElementById(t)) on the home page and again inside ❓ Help — every id it returns must be a key in FOLDED_TO.",
        "❓ Help: the contents strip has 🗂 Policies and 🧬 Baseline, with 📄 Create documentation, 🗄 Backup (JSON), 🎚 Set Policy state and 🧩 Baseline (Joey Verlinden) as indented sub-chips under them; clicking a sub-chip scrolls to that h5; the scroll-spy highlights sensibly on the way down. Ctrl+F for Create documentation still finds the section. 🔢 Tool numbers lists the four folded numbers with their hosts and reads T01 to T39.",
        "⌘K: type documentation, backup, Joey and Set policy state — each offers the folded tool with a hint naming the host and where inside it (the action bar / the catalog picker), and Enter opens the host. T02, T04, T05 and T11 as numbers do the same; T01 and T10 still answer to 1 and 10.",
        "Browser Back still walks the screens — screen-list and screen-baseline are unchanged in HISTORY_SCREENS.",
        "?demo=1 end to end: open 🗂 Policies, run Documentation and Backup with nothing selected, switch 🧬 Baseline between the two catalogs, then reload and confirm the What's-new overlay shows 25338 once.",
      ],
      carveout: "The Help fold notes, the 🔢 Tool numbers list and the R52 card all say build 25338, which is a BETA number: on main rewrite each of them to the production build this lands in (the folded into this tool — build NNNNN spans, the four bullets in Tool numbers, the until build 25338 clauses in the 🗂 Policies and 🧬 Baseline sections, and R52's data-shipped plus its live · build NNN tag). The FOLDED_TO comments in js/app.js and the version.js folded fields keep the beta number — they record when the fold happened on this channel, not what a customer reads. Prove the port: grep -n 'build 25338\\|25338' index.html on main must return nothing.",
      files: ["index.html", "js/app.js", "js/guide.js", "js/changelog.js", "js/promote.js", "js/version.js", "css/app.css"],
    },
    {
      n: 172,
