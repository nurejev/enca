...
// ======================================================================
const Render = (() => {
  // The on-screen policy marks. These used to hard-code assets/logo-mark-*.svg,
  // which quietly defeated the whole point of js/branding.js: a fork with its
  // own mark still showed ours on every policy card. Export paths are separate
  // and stay neutral — they carry the CUSTOMER's tenant branding, never a
  // product logo (see opts.logo).
  //
  ...
  function brandMark(px, dark) {
    ...
    const style = B.logoWide
      ? `height:${px}px;width:auto;max-width:${px * 4}px;object-fit:contain`
      : "";
    return `<img src="${src}" ${B.logoWide ? "" : `width="${px}" height="${px}"`} style="${style}" alt="">`;
  }
  ...
  function summaryCard(p, selected) {
    ...
    return `<div class="scard" data-open="${esc(p.id)}">
      <div class="scard-top">
        <div class="scard-ic">${brandMark(24)}</div>
  ...
  function card(p, tenantName, opts = {}) {
    ...
    const headLogo = opts.export
      ...
      : brandMark(30, true);
    // The compact card carries the policy id and this one did not, so the
    // moment somebody opened a policy to look at it properly the one string
    // that identifies it went away — and this is the card that gets exported,
    // screenshotted and pasted into a ticket. It is the only identifier that
    // survives a picture: two policies can share a name, a version and a
    // persona, and the portal is reached by id.
    const idLine = `<span class="mini pcard-id">Policy ID: <code>${esc(p.id)}</code></span>`;
    const foot = opts.export
      ? `<span class="mini">Conditional Access documentation${tenantName ? " · " + esc(tenantName) : ""} · ${new Date().toISOString().slice(0, 10)}</span>${idLine}`
      : `<span class="mini">Generated by ${esc(Brand.stamp)} · ${new Date().toISOString().slice(0, 10)}</span>${idLine}
         <button class="btn" data-png="${esc(p.id)}">↓ Save PNG</button>`;
