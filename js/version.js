...
const TOOL_VERSIONS = {
  ...
  toolGroupUse:     { v: "0.2", note: "BETA — where an Entra group is actually used: 22 services across Entra ID (nesting, administrative units, directory roles incl. PIM eligibility, enterprise apps, group-based licensing, authentication methods policy, Conditional Access, access packages, admin-consent reviewers, registration state), Intune (enrolment restrictions, compliance, configuration profiles, scripts, app protection, app configuration, app assignments, Autopilot, Windows update profiles), Microsoft 365 (M365 group / Team) and Azure RBAC across subscriptions and management groups; one group or user, or a tenant-wide sweep with an unused-groups view; parent-group inheritance shown per hit; per-area consent; Markdown, standalone HTML and CSV export. 0.2: sweep rows open a popup built from the sweep data instead of re-scanning (with Deep analyze for parent expansion), HTML report added, Intune scripts now request DeviceManagementScripts.Read.All and entitlement management uses single-level expands with a candidate ladder (both were failing), and every failure says which permission and which role it needs. After Jasper Baes' Microsoft Cloud Group Analyzer" },
