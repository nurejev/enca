...
const PimBaseline = (() => {
  ...
  function toOrchestrator(cat, opts = {}) {
    ...
    if (!(opts.breakGlass || []).length) out._protectedNote = `Add the break-glass accounts (${(cat.protected && cat.protected.pattern) || "BG-*"}) to ProtectedUsers — the script finds them by name.`;
    return out;
  }
