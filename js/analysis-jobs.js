  ...
  function pump() {
    while (active < 2 && queue.length) {
      ...
      try {
        ...
        worker.onmessage = e => e.data.error ? finish(new Error(e.data.error)) : e.data.progress ? job.onProgress?.(e.data.progress) : finish(null, e.data.result);
        worker.onerror = e => finish(new Error(e.message || "Analysis worker failed; no result was published"));
        job.signal?.addEventListener("abort", abort, { once: true });
        worker.postMessage({ kind: job.kind, args: job.args });
      } catch (e) { finish(e); }
    }
  }
